...
from hat.engine.predictor import Predictor, launch
...
def _ddp_pred_main(gpu_id, *args):
    ddp_predictor = Predictor(
        model=ToyModel(
...
def test_ddp_predictor_fit():
    seed_everything(os.getpid())
    launcher = launch

    device_ids = list(range(torch.cuda.device_count()))
    dist_url = "auto"
    launcher(_ddp_pred_main, device_ids, dist_url)
