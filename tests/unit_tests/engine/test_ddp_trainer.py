...
def test_amp_fit(enable_amp_dtype):
    ...
    launcher(_amp_main, device_ids, dist_url, args=(enable_amp_dtype,))
...
def _part_ddp_main(gpu_id, *args):
    cfg = copy.deepcopy(trainer_cfg)
    cfg["device"] = gpu_id
...
@pytest.mark.serial_task
def test_part_ddp_fit():
    seed_everything(os.getpid())
    launcher = build_launcher(trainer_cfg)

    device_ids = list(range(torch.cuda.device_count()))
    dist_url = "auto"
    launcher(_part_ddp_main, device_ids, dist_url)
...
def test_batch_transforms_fit():
    ...
    device_ids = list(range(torch.cuda.device_count()))
    dist_url = "auto"
    launcher(_batch_transforms_main, device_ids, dist_url)
...
def _resume_main(
    ...
    *args,
    ...
    num_iters = cfg["num_steps"] if stop_by == "step" else cfg["num_epochs"]

    ckpt_file = os.path.join(
        ckpt_dir,
        "checkpoint-%s.pth.tar"
        % (
            "step-%d" % (num_iters - 1)
            if stop_by == "step"
            else "epoch-%04d" % (num_iters - 1),
        ),
    )
    cfg_resume["model_convert_pipeline"] = dict(
        type="LoadCheckpoint",
    ...
    resumed_optimizer = resumed_trainer.optimizer

    assert resumed_optimizer
    if resume_optimizer:
        # ----------- check optimizer -------------
        assert len(resumed_optimizer.state_dict()["state"]) == len(
            checkpoint_state["optimizer"]["state"]
        )

        assert len(resumed_optimizer.state_dict()["param_groups"]) == len(
