...
def _clone_batch(batch, keys=None, gpu=None):
    if keys is None:
        keys = batch.keys()
    else:
        keys = _as_list(keys)
...
def test_all_task_forward_backward_step():
    ...
    batches = [
        [_clone_batch(inputs, None, gpu)]  # inputs, refer GraphModel.forward()
    ]
    _run_step(batches, model, is_train=True, optimizer=optimizer)

    # zero_grad -> fw_all -> bw_all -> fw_all -> bw_all -> step
    batches = [
        [_clone_batch(inputs, None, gpu)],
        [_clone_batch(inputs, None, gpu), out_names],
    ]
...
def test_one_task_forward_backward_step():
    ...
    batches = []
    for n in out_names:
        batches.append([_clone_batch(batch, ["img", n + lb_suffix], gpu), n])
...
def _run_partial_task(cfg, model, gpu):
    ...
    for i in range(len(out_names) - 1):
        ...
        lb_keys = ["img"] + [t + lb_suffix for t in tasks]
        batches.append(
            [
                _clone_batch(
                    batch, lb_keys, gpu
...
def test_dp_model():
    device_ids = list(range(min(2, torch.cuda.device_count())))
    dist_url = "auto"
    launcher = get_launcher("data_parallel_trainer")
    launcher(_dp_main, device_ids, dist_url)


@pytest.mark.skipif(hatbc is None, reason="need hatbc")
def test_get_sub_modules():
    cfg = _load_cfg()
    model = build_from_registry(cfg["test_traceable_model"])
