...
from hat.models.losses.traj_pred_loss import BehavHeadLoss, SoftMultipathLoss
from hat.models.task_modules.traj_pred.heads import (
...
from tests import SD_ALGO_BUCKET_PATH, SD_AlGORITHM_BUCKET_PATH
...
def gen_example_multipath_model_v2(
    ...
    model_heads = OrderedDict(
        ...
        behav_head=BasicBehavDecoder(
            ...
            post_process=post_process_dict["behav_head"]
            if "behav_head" in post_process_dict
            else None,
        ),
    )
...
def test_multipath_structure(version=2):
    ...
    tdt_dir = os.path.join(
        SD_ALGO_BUCKET_PATH,
        TRAJ_BEHAV_PERREC_PREFIX,
        "H3165_20220421_D/20220421-081209_224/traj_process_multitask/20_0.8.4_1_0.0.1_12_70_3_70_50contextframes_dist_restriction/hdfs_save/H3165_csv_trans/tdt/concat_tdt_v0.0_patch",  # noqa: E501
    )
    tdt_yaml_dir = os.path.join(
        SD_ALGO_BUCKET_PATH,
        TRAJ_BEHAV_PERREC_PREFIX,
        "H3165_20220421_D/20220421-081209_224/traj_process_multitask/20_0.8.4_1_0.0.1_12_70_3_70_50contextframes_dist_restriction/hdfs_save/H3165_csv_trans/tdt/concat_tdt_v0.0_patch/tdt_meta.yaml",  # noqa: E501
    )
    img_dir = SD_ALGO_BUCKET_PATH
    ...
    if version == 1:
        ...
        add_keys = [
            ...
            "behav_head_lon_behav_probs",
        ]
        traj_output_heads = ["traj_head"]
        behav_output_heads = ["behav_head"]
    ...
    for head_name in behav_output_heads:
        assert list(model_output[head_name + "_lat_behav_probs"].shape) == [
            num_obj,
            3,
            1,
            1,
        ]
        assert list(model_output[head_name + "_lon_behav_probs"].shape) == [
            num_obj,
            3,
            1,
            1,
