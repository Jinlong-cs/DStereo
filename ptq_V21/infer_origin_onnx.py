...
def postprocess(data):
    disp, spx, initdisp, initspx = data
    init_disp = np.sum(initdisp*initspx, axis=1, keepdims=False)[0]
    infer_disp = np.sum(disp*spx, axis=1, keepdims=False)[0]

    return np.ascontiguousarray(infer_disp)

def predict(sess, file, thr=0.09, maxdisp=192, color=(252,247,192), left_img_path="left.png", right_img_path="right.png", result_path="float_onnx_infer_result.png", preprocess_fn = None):
...
if __name__ == "__main__":
    ...
    for file in tqdm.tqdm(os.listdir(args.left_img)):
        bboxes = predict(sess, file, thr=0.09, left_img_path=args.left_img, right_img_path=args.right_img, result_path=args.result_path, preprocess_fn=preprocess)