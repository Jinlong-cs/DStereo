...
def predict_quant(args, sess, file, thr=0.09, maxdisp=192, color=(252,247,192), left_img_path="left.png", right_img_path="right.png", result_path="float_onnx_infer_result.png", preprocess_fn = None):
    ...
    right_input, origin_right = preprocess_fn(os.path.join(right_img_path, file))
...
def preprocess(p, crop_width=352, crop_height=640):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--left_img', type=str, default='/docker-mount/Discover/StereoEstimation/DStereo_V2.3/ptq_V21/ballcar_calib_data/left/', help='left image path')
    parser.add_argument('--right_img', type=str, default='/docker-mount/Discover/StereoEstimation/DStereo_V2.3/ptq_V21/ballcar_calib_data/right/', help='right image path')
    parser.add_argument('--save_npy', action='store_true', default=True, help='save npy')
    parser.add_argument('--save_npy_path', type=str, default='/docker-mount/Discover/StereoEstimation/DStereo_V2.3/ptq_V21/calib_data_ballcar/', help='save npy path')
    ...
    left_images = [
        p
        for p in os.listdir(args.left_img)
        if os.path.splitext(p)[1].lower() in exts
        and os.path.isfile(os.path.join(args.left_img, p))
    ]
    paired_images = [
        p for p in left_images if os.path.isfile(os.path.join(args.right_img, p))
    ]
    print(f"Found {len(left_images)} left images, paired {len(paired_images)}.")

    # 处理 ptq_num 指定数量的图片
    images = paired_images[:args.ptq_num] if args.ptq_num > 0 else paired_images
