...
# 读取配置文件
with open('/workspace/input/task.json', 'r') as f:
...
def get_gpu_count():
    try:
        output = (
            subprocess.check_output(["nvidia-smi", "-L"]).decode().strip().split("\n")
        )
        return [i for i in range(0,len([line for line in output if line.strip()]))]
    except:
        return [0]
...
task_name = "DStereoTask"
data_num_workers = 4
march = March.BAYES_E
ckpt_dir = "/workspace/output/%s" % task_name
...
train_batch_size_per_gpu = task_config.get("train", {}).get("batch_size", 8)
test_batch_size_per_gpu = 1
log_freq = 10
sync_bn = True
device_ids = get_gpu_count()
...
base_lr = 0.001
# num_steps = 100000
num_steps = task_config.get("train", {}).get("epochs", 100) 

print("task_name:",task_name)
print("GPU Count:",device_ids)
print("Batch Size:",train_batch_size_per_gpu)
print("num_steps:",num_steps)
print("Pre Train Ckpt Path:",checkpoint_path)
print("Ckpt Output Dir:",ckpt_dir)
...
float_predictor = dict(
    ...
    callbacks=[
        # dict(type="SaveCalibdata", output_dir="output/calib_data",),
        dict(type="SaveDisp", output_dir="output/predict_result",task_name="predict_result"),
        stat_callback,
        val_metric_updater,
    ],
