...
def find_closest_timestamps(data_dict, target_timestamp, start_=0):
    ...
    for idx, pose in enumerate(data_dict[start_:]):
        ...
        if target_datetime == pose_time:
            return True, list(data_dict[idx].values())[0], idx, list(data_dict[idx].values())[0], idx, 0.
        elif target_datetime > pose_time:
            if history_clost_pose is None:
                history_clost_pose = pose
                history_clost_pose_idx = idx
            else:
                if abs(target_datetime - pose_time) < abs(target_datetime - datetime.fromtimestamp(np.float64(list(history_clost_pose.keys())[0]))):
                    history_clost_pose = pose
                    history_clost_pose_idx = idx
        else:
            ...
            return True, list(history_clost_pose.values())[0],  history_clost_pose_idx, list(future_clost_pose.values())[0], future_clost_pose_idx, np.float64(list(future_clost_pose.keys())[0]) - np.float64(list(history_clost_pose.keys())[0])
            # else:
            #     assert False
            #     if abs(target_datetime - pose_time) < abs(target_datetime - datetime.fromtimestamp(float(list(future_clost_pose.keys())[0]))):
            #         future_clost_pose = pose
            #         future_clost_pose_idx = None
...
def load_odom(pose_file):
    ...
    return pose_dict    # [int(len(pose_dict) * 0.1) : int(len(pose_dict) * -0.1)]
...
def load_pcd(pcd_path):
    ...
    # print("load %d points from %s" % (num_points, pcd_path))
    points = []
...
def interploate_pose(pre_pose, next_pose, time_inter, return_pose=False):
    slerp_func = Slerp([np.float64(pre_pose[0]), np.float64(next_pose[0])], RR.from_quat([pre_pose[-4:], next_pose[-4:]]))

    trans1 = np.array([float(i) for i in pre_pose[1:4]])
    trans2 = np.array([float(i) for i in next_pose[1:4]])
    trans_inter = interpolate_translation(np.float64(pre_pose[0]), trans1, np.float64(next_pose[0]), trans2, time_inter)
    if return_pose:
...
def load_pose_revise(pose_file, key_frame_pose):
    ...
    for line in lines:
        txt_info = line.strip("\n").split(" ")
        # 判断是否存在keyframe pose，如果存在则使用keyframe替代。
        flag, current_pose = replace_norm_pose_to_keyframe(copy.deepcopy(txt_info), key_frame_pose)
        # 不是的话，继续根据前一个pose，更新当前pose
        if not flag:
            if len(pose_dict) == 0:
                continue
            current_pose = pose2rotation(copy.deepcopy(txt_info)) @ np.linalg.inv(pose2rotation(pre_pose)) @ pose2rotation(copy.deepcopy(pre_pose_revise))
            current_pose = rotation2pose(current_pose)
            current_pose = [txt_info[0],] + current_pose
...
def depth2disp(depth, baseline_rectified, f_rectified, verbose=False):
    ...
    if verbose:
        mask_dis = (disp >= 1.)
        print("disparity density: %d / %d = %0.2f" % (mask_dis.astype(np.int32).sum(), depth.size, float(mask_dis.astype(np.int32).sum()) / depth.size))
    # disp = (disp * 255.0).astype(np.uint16)
...
def disp2gray(disp, disp_max, disp_min):
    ...
    disp *= 255
    disp = np.round(disp).astype(np.uint8)[..., None]
    disp = cv2.applyColorMap(disp, cv2.COLORMAP_BONE)#[:,:,::-1]
    disp[mask] = (0, 0, 0)

    return disp

def uncert2rgb(disp):
    disp = np.clip(disp, 0, 1.0)  # 近处是0， 远处320
    disp *= 255
    disp = np.round(disp).astype(np.uint8)[..., None]
...
def depth2rgb(depth, depth_max, depth_min):
    ...
    depth = np.clip(depth, depth_min, depth_max)  # 近处是0， 远处320
    depth = (depth - depth_min) / 4.34 * 0.7 + 0.25
    depth = abs(depth - 1)
    depth *= 255
    depth = np.round(depth).astype(np.uint8)[..., None]
    depth = cv2.applyColorMap(depth, cv2.COLORMAP_JET)#[:,:,::-1]
    depth[mask] = (0, 0, 0)
...
def view_infer_result(left, right, pred, disp_gt=None, maxdisp=192, color=(252,247,192)):
    ...
    scend = np.hstack((view_disp, view_gt)) if disp_gt is not None else np.hstack((view_disp, view_disp * 0))
