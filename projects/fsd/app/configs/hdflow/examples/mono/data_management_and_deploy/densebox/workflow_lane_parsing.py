...
args = parse_args()
assert (
    args.workflow_aidi_dataset_ids is None
), "parsing do not support using aidi_dataset_ids as inputs"
...
), "Parsing/LaneParsing Do not support keep_duplicate"  # noqa
assert (
    task_configs.is_merge is False
), "DataDeployTasksCfgs.is_merge only available for parsing"
cache_root = (
...
#  make input configs
# ---------------------------------------------------------------------------------
# unused key: see https://gitlab.hobot.cc/auto/perception/ad/hdflow/-/blob/master/hdflow/auto_dp/annotation.py#L452  # noqa
query_conds = (
    [
        LabelClientQueryTags.from_dict(i)
        for i in json.loads(args.workflow_aidi_label_query_jdict)  # noqa
    ]
...
default_viz_fn_config = dict(
    ...
    task_type="lane_parsing",
    save_path=viz_result_path,
    lane_parsing_color_map_config=task_configs.parsing_label_map,
)
