...
from traffic_light_lens_det import (
    num_color_cls,
    num_eval_color,
    num_eval_type,
    num_type_cls,
)
from utils import reformat_attr_det_to_aidi_eval, reformat_tl_cls_to_aidi_eval
...
for task_type, all_datasets in dataset_ids.items():
    for dataset, task_cfgs in all_datasets.items():
        ...
        reformat_output_fn = (
            reformat_tl_cls_to_aidi_eval
            if "classification" in task_type
            else reformat_attr_det_to_aidi_eval
        )
        reformat_out_fn_kwargs = (
            ...
            if "classification" in task_type
                ...
                num_color_cls=num_color_cls,
                num_type_cls=num_type_cls,
                num_eval_color=num_eval_color,
                num_eval_type=num_eval_type,
            )
        )
        if task_cfgs[0].endswith("_classification"):
            prediction_tags = {}
