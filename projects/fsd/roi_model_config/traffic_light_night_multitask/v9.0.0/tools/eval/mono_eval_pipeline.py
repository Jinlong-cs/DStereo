...
def run_submit(
    ...
    k8s_config = f"{os.path.dirname(__file__)}/../mono_tl_cluster_cfg.yaml"
    ...
    cmd = (
        ...
        + f" --hat-tl-model_setting {model_setting}"
    ...
    if model_name_postfix:
        cmd += f" --hat-tl-model-name-postfix {model_name_postfix}"

    if model_version:
        cmd += f" --hat-tl-model-version {model_version}"

    if eval_postfix:
        cmd += f" --hat-tl-prediction-name-suffix {eval_postfix}"

    eval_model_checkpoint = os.getenv("HAT_TL_MODEL_CHECKPOINT", None)
    prediction_name = os.getenv("HAT_TL_PREDICTION_NAME_SUFFIX", None)
    # 添加环境变量
    job_list = [
        f"export HAT_TL_MODEL_VERSION={model_version}",
        f"export HAT_TRAINING_STEP={stage}",
        f"export HAT_TL_MODEL_NAME_POSTFIX={model_name_postfix}",
        f"export HAT_TL_PREDICTION_NAME_SUFFIX={prediction_name}",
    ]
    if eval_model_checkpoint is not None:
        job_list += [f"export HAT_TL_MODEL_CHECKPOINT={eval_model_checkpoint}"]
    # 添加评测脚本
    job_list += [cmd]
    eval_config.update(
        dict(
