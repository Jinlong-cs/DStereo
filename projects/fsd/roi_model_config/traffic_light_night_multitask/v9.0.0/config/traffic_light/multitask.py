...
# -------------------------- stages --------------------------
pretrain_checkpoint = "http://fm-fan-lv.tcloud.hogpu.cc/plat_gpu/hobot-dag-3175472_traffic-light-mono-cn-2pe-day-multitask--v0-2-0-fan-lv-20230626-170756/output/models/traffic_light_multitask/with_bn-checkpoint-last.pth.tar"  # noqa
stage_extend_params = dict(
    with_bn=dict(
        checkpoint_path=pretrain_checkpoint,
        allow_miss=True,
        ignore_extra=True,
        state_dict_update_func=filter_head,
        # verbose=0, # 1
...
deploy_inputs = dict(
    img=torch.randn((deploy_batch, 3, *input_size)),
)
if mask_in_bpu:
    deploy_inputs.update(
        mask_width=torch.ones((deploy_batch, 1, 1, input_size[0])),
        mask_height=torch.ones((deploy_batch, 1, 1, input_size[1])),
    )
...
compile_cfg = dict(
    ...
    input_layout="NCHW",
    output_layout="NCHW",
    input_source=["resizer", "ddr", "ddr"] if mask_in_bpu else ["resizer"],
