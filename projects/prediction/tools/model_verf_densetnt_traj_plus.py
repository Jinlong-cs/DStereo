...
# pkl_dataset_path = "/horizon-bucket/SD_Algorithm/11_perception_prediction/02_user/shiqi.tan/traj_dataset_pkl_path/fus_version-hdmap_dataset_version1/fus_data_viz2.pkl"  # noqa: E501
...
pre_file_name = "/jfs-public/users/shiqi.tan/results/densetnt_task16_stage1_regress_addodd_20231106/checkpoint/densetnt_traj_release_v1.0.1_20231124_bpu2.5/"  # noqa: E501
qat_ckpt_path = pre_file_name + "qat-checkpoint-best.pth.tar"
pt_model_path = (
    pre_file_name
    + "densetnt_v1.0.1_bpu2.5_16_obstacles_e7f2b1e0fd2e9651704a8b194d3a4f1b.pt"  # noqa: E501
)
hbm_path = (
    pre_file_name
    + "densetnt_v1.0.1_bpu2.5_16_obstacles_c5e5d5f0d1688423270f6ea271c21919.hbm"  # noqa: E501
)
...
if bpu == 2.2:
    ...
    verf_files = [
        "hbdk_output1_feature__normal_head_aten_permute_2_elem_float.txt",  # noqa: E501
        "hbdk_output0_feature__normal_head_hz_cat_3_elem_float.txt",  # noqa: E501
    ]
else:
...
hbdk_cmd += (
    f"--model-input {attention_mask_file},{state_vectors_file},"  # noqa: E501
)
...
if workflow["bpu"]:
    ...
    for info in predict_trajs_for_val_infos:
        ...
        predict_trajs_for_val.append(tmp)
    predict_trajs_for_val = np.array(predict_trajs_for_val).reshape(
        -1, cfg.gt_traj_len, 2, max(cfg.k_values)
    )
...
# 差分轨迹转正常轨迹
predict_trajs_for_val = torch.tensor(predict_trajs_for_val)
predict_goal_scores = torch.tensor(predict_goal_scores)

predict_goal_scores = F.softmax(predict_goal_scores, dim=3)

indices = torch.argsort(predict_goal_scores, dim=3, descending=True)

predict_trajs_for_val = predict_trajs_for_val.gather(
    3, indices.repeat(1, 12, 2, 1)
)
predict_goal_scores = predict_goal_scores.gather(3, indices)

if "diff_fut_trajs" in model_in:
    predict_trajs_for_val = torch.cumsum(predict_trajs_for_val, dim=2)
trajectories = predict_trajs_for_val[
    : real_struct_road_feats.shape[0], :, :, :
]
trajectories = trajectories.permute(0, 3, 1, 2)
...
ret["all_vcs_score"] = predict_goal_scores
