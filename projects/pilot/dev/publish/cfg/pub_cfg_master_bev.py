...
publish_name = BEVModelReleaseName.master_bev
...
# ======== 7V 需要编译的模型 ========
bev_7v_ckpt = "http://svcspawner.bcloud-2nd.hobot.cc/user/homespace/wenyuan.zeng/plat_gpu/hobot-dag-6804926_pilot-multitask-bev-7v-pilot5-1-master-v3-0-0-20240104-214129/output/models/pilot_multitask_bev_7v/qat-checkpoint-last-319da8b3.pth.tar"  # noqa

models[BEVModelName.bev_multitask_stage1_fov120] = dict(
    cfg_path=os.path.join(cfg_dir, "bev_7v/multitask.py"),
...
bev_7v_torch_native = ["True"] * bev_7v_num_vcs_plane_heights * bev_7v_num_view
# stage1 feat
bev_7v_input_key += [f"pred_segs_frame0_{i}" for i in range(7)]
bev_7v_input_shape += (
    ["1x4x128x240"] + ["1x4x160x240"] * bev_7v_num_side_view + ["1x4x128x240"]
)
bev_7v_torch_native += ["False"] * bev_7v_num_view


models[BEVModelName.bev_multitask_stage2] = dict(
    cfg_path=os.path.join(cfg_dir, "bev_7v/multitask.py"),
    model_setting=BEVModelSetting.pilot51_master,
    input_shape="^".join(bev_7v_input_shape),
    input_type="dict",
    input_source="ddr",
    input_key="^".join(bev_7v_input_key),  # noqa
    torch_native="^".join(bev_7v_torch_native),
    jobs_num=jobs_num,
    extra_args="--max-time-per-fc 1000 --balance 1",
    output_layout="NCHW",
    input_layout="NCHW",
    ...
    update_cfg=dict(
        bev_3d_vehicle=dict(
            score_threshold=0.145,
            roi_score_threshold=[0.145] * 7,
        ),
        bev_3d_vrumerge=dict(
            score_threshold=[0.185, 0.165],
            roi_score_threshold=[0.185, 0.165],
        ),
        online_mapping=dict(
            lane=0.57,
            roadedge=0.45,
    ...
    split_flag=BEVModelName.bev_multitask_stage2,
    override=True,
    debug=True,
    dump_cmp_info=True,
    model_address=bev_7v_ckpt,
...
# ======== 5V 需要编译的模型 ========
bev_5v_ckpt = "http://svcspawner.bcloud-2nd.hobot.cc/user/homespace/zixiang.pei/plat_gpu/hobot-dag-6348571_pilot-multitask-bev-5v-pilot5-1-master-v3-release-20231225-213757/output/models/pilot_multitask_bev_5v/qat-checkpoint-last-f234d458.pth.tar"  # noqa
...
models[BEVModelName.bev_multitask_small_stage2] = dict(
    ...
    update_cfg=dict(
        ...
        bev_3d_vrumerge=dict(
            score_threshold=[0.25, 0.36],
            roi_score_threshold=[0.25, 0.36],
        ),
        online_mapping=dict(
            lane=0.7,
            roadedge=0.55,
