...
from projects.pilot.configs.bev_7v.data_loaders import data_loader
from projects.pilot.configs.bev_7v.models import deploy_model, model  # noqa
from projects.pilot.configs.bev_7v.schedule import (
    base_lr,
    interval_by,
    num_steps,
    save_interval,
...
lr_callback = dict(
    type="CosLrUpdater",
    max_steps=(num_steps - warmup_steps) // (num_machines * batch_size_factor),
    warmup_by="step",
    warmup_len=warmup_steps // (num_machines * batch_size_factor),
...
base_trainer = dict(
    ...
    optimizer=dict(
        ...
        params={},
        lr=base_lr * (num_machines * batch_size_factor),
        weight_decay=0.01,
    ),
    batch_processor=batch_processor,
    stop_by="step",
    num_steps=num_steps // (num_machines * batch_size_factor),
...
# -------------------------- training ----------------------------
pretrain_checkpoint = "http://svcspawner.bcloud-2nd.hobot.cc/user/homespace/feng02.li/plat_gpu/hobot-dag-6543222_pilot-multitask-bev-7v-pilot5-1-master-v3-0-0-Pretrain-20231229-082616/output/models/pilot_multitask_bev_7v/float_freeze_bn-checkpoint-last-55997674.pth.tar"  # noqa
...
if split_flag is not None:
    ...
    if split_flag == BEVModelName.bev_multitask_stage2:
