...
# from hat_internal.visualize import map_views_to_visual_type
...
merge_crosspoint = False if deploy_mode else True
...
om_merge_solid_dash_cfg = {
    "dist_thresh": 0.55,  # unit: m
    "length_thresh": 2.0,  # unit: m
    "split_half_thresh": 1.5,  # unit: m
    "split_diff_thresh": 3,  # unit: m
    "split_ratio": 0.85,
}
om_head_groups_update = {
    "lane": {
        "cls_thr": model_thresh.get(task_name, {}).get("lane", 0.47),
        "loss_type": "focal_loss",
    },
    "roadedge": {
        "cls_thr": model_thresh.get(task_name, {}).get("roadedge", 0.4),
        "loss_type": "focal_loss",
    },
}
...
if om_merge_solid_dash_cfg is not None:
    om_head_groups_update["lane_dashed"] = {
        "global_auto_class_weight": [1.0, 2.0],
        "cls_remap": {
            "solid": 0,
            "dotted": 1,
            ...
            "ldotted_fishbone": 1,
            "boots_dots": 1,
            "left_solid_right_dash": 2,
            "left_dash_right_solid": 3,
            "other": -1,
            "unknown": -1,
        },
        "cls_list": [
        ...
        "cls_colors": ["red", "blue", "yellow", "green"],
    }
...
merge_instance_cfg = {
    "normal_distance_thresh": 0.45,  # unit: m
    "double_distance_thresh": 0.185,  # unit: m
    "direction_thresh": 10,  # unit: degree
}
...
metrics += [
    ...
    dict(type="LossShow", name="loss_cls_dice_all"),
...
if merge_crosspoint:
    metrics.extend(
        [
            ...
            dict(type="LossShow", name="loss_cpts_dice"),
        ]
    )
    per_metric_patterns.extend(
        [
            ...
            dict(
                label_pattern=None,
                pred_pattern=f"^.*{task_name}_head.*loss_cpts_dice$",
            ),
        ]
    )
if adaptive_loss_weight:
    metrics += [
        ...
        dict(type="LossShow", name="loss_s_cls_dice"),
    ...
    if merge_crosspoint:
        metrics.extend(
            [
                ...
                dict(type="LossShow", name="loss_s_cpts_dice"),
            ]
        )
        per_metric_patterns.extend(
            [
                ...
                dict(
                    label_pattern=None,
                    pred_pattern=f"^.*{task_name}_head__loss_s_cpts_dice$",
                ),
            ]
        )
for om_head, head_infos in om_head_groups.items():
...
def get_inputs():
    inputs = dict(
        om_target=dict(
            lane=dict(
                cls=torch.zeros(1, 2, *head_out_size, dtype=torch.int64),
                instance=torch.zeros(1, 2, *head_out_size, dtype=torch.int64),
                r=torch.zeros(1, 2, *head_out_size, dtype=torch.float64),
                sin=torch.zeros(1, 2, *head_out_size, dtype=torch.float64),
                cos=torch.zeros(1, 2, *head_out_size, dtype=torch.float64),
                dilate=torch.zeros(1, 2, *head_out_size, dtype=torch.int64),
                weight=torch.zeros(1, 2, *head_out_size, dtype=torch.float64),
                main_roi_mask=torch.zeros(
                    1, 2, *head_out_size, dtype=torch.float64
                ),
                lane_direction=torch.full(
                    (1, 2, *head_out_size), -1, dtype=torch.int64
                ),
                lane_color=torch.full(
                    (1, 2, *head_out_size), -1, dtype=torch.int64
                ),
                lane_wide=torch.full(
                    (1, 2, *head_out_size), -1, dtype=torch.int64
                ),
            ...
            roadedge=dict(
                cls=torch.zeros(1, *head_out_size, dtype=torch.int64),
                instance=torch.zeros(1, *head_out_size, dtype=torch.int64),
                r=torch.zeros(1, *head_out_size, dtype=torch.float64),
                sin=torch.zeros(1, *head_out_size, dtype=torch.float64),
                cos=torch.zeros(1, *head_out_size, dtype=torch.float64),
                dilate=torch.zeros(1, *head_out_size, dtype=torch.int64),
                weight=torch.zeros(1, *head_out_size, dtype=torch.float64),
                main_roi_mask=torch.zeros(
                    1, *head_out_size, dtype=torch.float64
                ),
                roadedge_subtype=torch.full(
                    (1, *head_out_size), -1, dtype=torch.int64
                ),
            ),
        ),
    )
    if merge_crosspoint:
        inputs["crosspoint_target"] = dict(
            crosspoints=dict(
                x=torch.zeros(1, 1, *head_out_size, dtype=torch.float64),
                y=torch.zeros(1, 1, *head_out_size, dtype=torch.float64),
                cls=torch.zeros(1, 6, *head_out_size, dtype=torch.float64),
            ),
            changepoints=dict(
                x=torch.zeros(1, 1, *head_out_size, dtype=torch.float64),
                y=torch.zeros(1, 1, *head_out_size, dtype=torch.float64),
                cls=torch.zeros(1, 1, *head_out_size, dtype=torch.float64),
            ),
        )
...
# ------------------ DATASET SETTING -------------------
train_data_version = "v5_6_0_mixed_views_select_wide_temporal_v2"
val_data_version = [
    "v5_0_0_11v_ls_driving",
    "v5_0_0_11v_driving",
    "v6_0_0_11v_driving",
    "v6_0_0_11v_driving_highway",
]
if pipeline_test:
...
om_horizontal_cfg = {
    ...
    "horizontal_ignore_roi": [
        [52.5, -22.4, 100.8, -10.5],
        [52.5, 10.5, 100.8, 22.4],
    ],
...
om_target = dict(
    ...
    merge_crosspoint=merge_crosspoint,
    om_horizontal_cfg=om_horizontal_cfg,
    merge_solid_dash_cfg=om_merge_solid_dash_cfg,
)
if merge_crosspoint:
...
om_transforms = [
    ...
    *om_target_list,
    ...
    common_transforms["ANCPrepareTempoDataBEV"],
...
def get_train_dataloader():
    ...
    base_train_data_file_list = [
        "om_dataset_driving_temporal.yaml",
        "om_dataset_parking_temporal.yaml",
