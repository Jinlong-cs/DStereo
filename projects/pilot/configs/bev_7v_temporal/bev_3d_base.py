...
bev_3d_stage2_output_resolution = (
    abs(vcs_range[2] - vcs_range[0]) / bev_3d_out_size[0],
    abs(vcs_range[3] - vcs_range[1]) / bev_3d_out_size[1],
)  # (height, witdh)
max_objs = 900
...
bev_3d_common_transforms = deepcopy(common_transforms)
bev_3d_common_transforms["ANCCollect3DV"]["gt_bev_3d_idx"] = 0
bev_3d_common_transforms["ANCCollect3DV"]["load_data_types"] = load_data_types
bev_3d_common_transforms["ANCMultiViewTargetGenerator"] = dict(
    type="ANCMultiViewTargetGenerator",
    occlusion_attribute=False,
    occlusion_attribute_dict=None,
    use_ignore_mask_img=False,
)
bev_3d_common_transforms["ANCApplyMaskOnImg"] = dict(
...
def get_metrics_patterns(
    ...
    if use_corner_loss:
        metrics.append(dict(type="LossShow", name=f"{task_name}_corner_loss"))
        per_metric_patterns.append(
            dict(
                label_pattern=None,
                pred_pattern=f"^.*{task_name}.*bev3d_corner_loss$",
            )
        )
...
def get_val_metrics(
    ...
    metric_key = list(deepcopy(metric_key))
    if eval_occlusion:
...
save_pack_infer = dict(
    ...
    prefix="bev_stage2_3d_obj_head_predict",
)


# ------------------------- DATASET ----------------------------------
def update_dataset_for_return_all(dataset, val_num_frames_per_iter):
    # update transform to obtain all gt for frames in clips
    for transform in dataset["transforms"]:
        if transform["type"] == "ANCCollect3DV":
            transform["num_frames_per_iter"] = val_num_frames_per_iter
        if (
            transform["type"] == "AddKeys"
            and "return_latest_flag" in transform["kv"]
        ):
            transform["kv"]["return_latest_flag"] = False
        if transform["type"] == "ANCBev3dTargetGenerator":
            transform["type"] = "ANCTemporalBev3dTargetGenerator"
            transform["reverse_anno"] = True
        if transform["type"] == "ANCResize3DV":
            transform["type"] = "ANCTemporalResize3DV"
        if transform["type"] == "ANCCrop3DV":
            transform["type"] = "ANCTemporalCrop3DV"
        if transform["type"] == "ANCToTensor3DV":
            transform["type"] = "ANCToTensorTemporal3DV"
        if transform["type"] == "ANCPrepareTempoDataBEV":
            transform["type"] = "ANCPrepareTempoALLFrameDataBEV"
        if transform["type"] == "ANCMultiViewTargetGenerator":
            transform["type"] = "ANCTemporalMultiViewTargetGenerator"
        if transform["type"] == "ANCApplyMaskOnImg":
            transform["type"] = "ANCTemporalApplyMaskOnImg"


def update_val_dataloaders_for_return_all(
    data_loaders, batch_size_per_gpu, num_frames_per_iter, all_length_of_clip
):
    # update sampler to restrict sequential clip data.
    for data_loader_i, length_of_clip in zip(data_loaders, all_length_of_clip):
        data_loader_i["sampler"] = dict(
            type=ANCDistributedGroupClipSampler,
            samples_per_gpu=batch_size_per_gpu,
            sub_clip_num=length_of_clip // num_frames_per_iter,
        )
