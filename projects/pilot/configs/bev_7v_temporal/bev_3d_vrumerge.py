...
for transform in bev_3d_transforms:
    if transform["type"] == "ANCCollect3DV":
        transform["load_data_types"].append("bev3d_vrumerge_pose")
        break
    ...
    if mode == "train":
        ...
        postprocess_modules.append(
            dict(
                ...
                node_name="bev_stage2_3d_vrumerge_dequant",
            )
        )
        if use_occlusion_attribute:
            postprocess_modules.insert(1, occlusion_hm_argmax)
        postprocess_modules.append(
            dict(
                type="AddDesc",
                per_tensor_desc=bev3d_desc,
                node_name="bev_stage2_3d_vrumerge_add_desc",
            )
        )
...
visualize_callbacks = [
    dict(
        ...
        task="bev_3d_vrumerge",
        prefix="bev_stage2_3d_vrumerge_head_predict",
