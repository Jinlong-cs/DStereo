...
from projects.pilot.configs.bev_7v_temporal.bev_arrow_roadmarking import (
    arrow_roi_vcs_range as roi_vcs_range,
)
from projects.pilot.configs.bev_7v_temporal.bev_arrow_roadmarking import (
    arrow_task_in_stride as task_in_stride,
)
from projects.pilot.configs.bev_7v_temporal.bev_arrow_roadmarking import (
    arrow_task_out_size as task_out_size,
)
from projects.pilot.configs.bev_7v_temporal.bev_arrow_roadmarking import (
...
dataset_name = "bev_arrow_roadmarking"

# ------------------ BEV arrow transformation setting -------------------

bev_discobj_arrow_stage2_output_resolution = (
...
# ----------------------- DATALODER---------------------------
data_yaml = os.path.join(dataset_dir, f"{dataset_name}_dataset.yaml")


def get_train_dataloader():
    url = os.path.join(dataset_dir, f"{dataset_name}_train_version.py")
...
def get_val_dataloader():
    val_url = os.path.join(dataset_dir, f"{dataset_name}_val_version.yaml")
...
if arrow_head_with_cls:
    ...
    head_channels = OrderedDict(
        ...
        pred_bev_discobj_hm_cls=num_classes,
...
task_val_metric_setting = copy.deepcopy(arrow_val_metric_setting)
arrow_decode_setting = {
    "topk": 20,
    "max_pool_kernel": 7,
    "do_ct_nms": True,
    "ct_dist_scale": 1.0,
...
bev_common_desc = dict(
    ...
    bev_stage2_output_resolution=bev_discobj_arrow_stage2_output_resolution,
...
def get_desc(
    ...
    apply_decode_setting = arrow_decode_setting
    apply_common_desc = copy.deepcopy(bev_common_desc)

    if arrow_head_with_cls:
        per_tensor_desc = [
            {
                "task": "bev_discobj_arrow",
                "output_name": "bev_discobj_arrow_heatmap_output",
                ...
                "ct_dist_scale": apply_decode_setting.get(
                    "ct_dist_scale", 1.0
                ),
            ...
            {
                "task": "bev_discobj_arrow",
                "output_name": "bev_discobj_arrow_classification_output",
                "properties": [
                    {
                        "channel_labels": [
    ...
    if use_psc_rot and N_steps_PSC_rot is not None:
        ...
        postprocess_rot_desc = {
            "task": "bev_discobj_arrow",
            "output_name": "bev_discobj_arrow_rot_output",
    ...
    per_tensor_desc += [
        {
            "task": "bev_discobj_arrow",
            "output_name": "bev_discobj_arrow_dimension_output",
        ...
        {
            "task": "bev_discobj_arrow",
            "output_name": "bev_discobj_arrow_ct_offset_output",
...
def get_model(mode):
    ...
    if mode == "train":
        ...
        bev_discobj_arrow_desc = get_desc(
            use_psc_rot=use_psc_rot,
        ...
        postprocess_modules.append(
            dict(
                type="AddDesc",
                per_tensor_desc=bev_discobj_arrow_desc,
            )
        )
        bev_head["postprocess"] = dict(
...
metrics, per_metric_patterns = get_metrics_patterns(
    task_name,
    head_with_cls=arrow_head_with_cls,
)
...
for dataset_key in _as_list(val_data_version):
    ...
    val_metrics = get_val_metrics(
        task_name=task_name,
        id2label=id2label,
        eval_category_ids=tuple(range(num_classes))[1:],
...
visualize_callbacks = [
    dict(
        ...
        output_dir=os.path.join(save_prefix, "visualize", "bev_arrow"),
        task="bev_arrow",
        ...
        camera_layouts=None,
        res_key2anno_name={
            "bev_arrow": "annos_bev_discrete_obj",
        },
        color_map=COLOR_MAP,
        id2label=ID2LABEL,
        visual_interval=1,
