...
use_box_clip = False
if "c385" in model_setting:
    use_box_clip = True

box_clip_ratio = (0, 0.15, 0, 0)
...
roi_feat_extractor = dict(
    ...
    canonical_level=5,
    aligned=None,
    node_name=f"{task_name}_roi_feat_extractor",
)
...
def get_model(mode):
    ...
    roi_feat_extractor_val_test = deepcopy(roi_feat_extractor)
    if use_box_clip:
        roi_feat_extractor_val_test.update(box_clip_ratio=box_clip_ratio)
        hm_postprocess.update(box_clip_ratio=box_clip_ratio)
    ...
    return dict(
        ...
        roi_module=dict(
            ...
            if mode == "train"
            else roi_feat_extractor_val_test,
            head=dict(
                type="ExtSequential",
                modules=[
                    dict(
            ...
            postprocess=hm_postprocess if "val" in mode else None,
        ),
    )
...
if lmdb_data:
    ...
    data_loader = dict(
        ...
        transform=[
            ...
            dict(
                type="PadPersonHeadGTBeforeLabelGenerator",
