...
def get_model(mode):
    model = dict(
        ...
        loss=dict(
            ...
            loss_custom_weight=loss_custom_weight_init
            if is_loss_custom
            else None,
...
inputs = dict(
    train=dict(
        heatmap=torch.zeros(
            (1, 1, actual_input_hw[0] // 4, actual_input_hw[1] // 4)
        ),
        box2d_wh=torch.zeros(
            (1, 2, actual_input_hw[0] // 4, actual_input_hw[1] // 4)
        ),
        dimensions=torch.zeros(
            (1, 3, actual_input_hw[0] // 4, actual_input_hw[1] // 4)
        ),
...
transforms = [
    ...
    dict(
        ...
        min_box_edge=7,  # original image size
        max_depth=max_depth,
        max_objs=max_objs,
        undistort_2dcenter=True,
        undistort_depth_uv=undistort_depth_uv,
    ...
    data_loader = dict(
        ...
        transform=[
            ...
            dict(
                ...
                pe_config=pe_config if is_with_pe else None,
                depth_min_option=depth_min_option,
                keep_meta_keys=["img_wh", "loss_custom_weight"],
            ),
        ],
    )
    if is_loss_custom:
