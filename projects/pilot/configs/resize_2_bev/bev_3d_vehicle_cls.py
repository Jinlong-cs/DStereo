...
cls_dimension = np.array(
    [[1.7761209, 1.8237954, 4.79461]] * num_classes
)  # Vehicle
cls_hm_kernel = {i: 9 for i in range(num_classes)}
...
val_data_paths = datapaths.multiview_vehicle_3d_detection.val_data_paths
...
def get_model(mode):
    bev_stage2_out_module = dict(
        ...
        postprocess=None,
        target=None,
        node_name="bev_stage2_3d_vehicle_cls_head",
    )
    ...
    if mode == "val":
        ...
        bev_stage2_out_module["postprocess"] = dict(
            ...
            node_name="bev_stage2_3d_vehicle_cls_decoder",
        )
    if mode == "test":
    ...
    if not seperate_stage2_among_bev_tasks:
        veh_backbone = bev_backbone
    else:
        veh_backbone = copy.deepcopy(bev_backbone)
        veh_backbone["node_name"] = "bev_veh_stage2_back_bone"
    if (
        ...
        veh_neck = copy.deepcopy(bev_neck)
        veh_neck["node_name"] = "bev_veh_stage2_neck"
...
def get_bev_desc():
    per_tensor_desc = [
        {
            "task": "bev_3d_vehicle_cls",
            "output_name": "bev_3d_vehicle_cls_heatmap_output",
            "score_threshold": model_thresh["bev_det_thresh"][task_name]
            if model_thresh
            else 0.2,
            "max_pool_kernel": 9,
        ...
        {
            "task": "bev_3d_vehicle_cls",
            "output_name": "bev_3d_vehicle_cls_dimension_output",
        ...
        {
            "task": "bev_3d_vehicle_cls",
            "output_name": "bev_3d_vehicle_cls_rot_output",
            "properties": [{"channel_labels": ["cos", "sin"]}],
            **bev_common_desc,
        },
        {
            "task": "bev_3d_vehicle_cls",
            "output_name": "bev_3d_vehicle_cls_ct_offset_output",
            "properties": [{"channel_labels": ["offset_x", "offset_y"]}],
            **bev_common_desc,
        },
        {
            "task": "bev_3d_vehicle_cls",
            "output_name": "bev_3d_vehicle_cls_loc_z_output",
