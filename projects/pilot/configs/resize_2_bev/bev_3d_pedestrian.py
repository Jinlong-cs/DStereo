...
real_3d_category_id_dict = get_real3d_category_id_map(task_name)
bev3d_target = dict(
    ...
    category2id_map=real_3d_category_id_dict,
    enable_ignore=use_ignore_mask,
)
bev_3d_transforms = [
    dict(
        type="ConvertMultiViewAnnoTo3DV",
        category_id_dict=real_3d_category_id_dict,
...
train_data_paths = datapaths.multiview_pedestrian_3d_detection.train_data_paths
val_data_paths = datapaths.multiview_pedestrian_3d_detection.val_data_paths
...
# -------------------------- MODEL --------------------------
train_inputs, val_inputs, test_inputs = get_inputs()
inputs = dict(
    train=train_inputs,
    val={},
    test=test_inputs,
...
def get_model(mode):
    bev_stage2_out_module = dict(
        ...
        loss=dict(
            ...
            loss_weights=dict(
                ...
                bev3d_rot=2.0 * loss_weight * task_loss_weight,
        ...
        postprocess=None,
        target=None,
        node_name="bev_stage2_3d_pedestrian_head",
    )
    ...
    if mode == "val":
        ...
        bev_stage2_out_module["postprocess"] = dict(
            ...
            node_name="bev_stage2_3d_pedestrian_decoder",
        )
    if mode == "test":
    ...
    indicator = "ped" if not merge_ped_and_cyc else "pedcyc"
    if not seperate_stage2_among_bev_tasks:
        ped_backbone = bev_backbone
    else:
        ped_backbone = copy.deepcopy(bev_backbone)
        ped_backbone["node_name"] = f"bev_{indicator}_stage2_back_bone"
    if (
        ...
        ped_neck = copy.deepcopy(bev_neck)
        ped_neck["node_name"] = f"bev_{indicator}_stage2_neck"
    return dict(
...
def get_bev_desc():
    per_tensor_desc = [
        {
            "task": "bev_3d_pedestrian",
            "output_name": "bev_3d_pedestrian_heatmap_output",
            "score_threshold": model_thresh["bev_det_thresh"][task_name]
            if model_thresh
            else 0.2,
            "max_pool_kernel": 3,
            "properties": [{"channel_labels": ["pedestrian"]}],
            **bev_common_desc,
        },
        {
            "task": "bev_3d_pedestrian",
            "output_name": "bev_3d_pedestrian_dimension_output",
            "cls_dimension": [1.6383535, 0.60629284, 0.5058226],
            "properties": [{"channel_labels": ["height", "width", "length"]}],
            **bev_common_desc,
        },
        {
            "task": "bev_3d_pedestrian",
            "output_name": "bev_3d_pedestrian_rot_output",
            "properties": [{"channel_labels": ["cos", "sin"]}],
            **bev_common_desc,
        },
        {
            "task": "bev_3d_pedestrian",
            "output_name": "bev_3d_pedestrian_ct_offset_output",
            "properties": [{"channel_labels": ["offset_x", "offset_y"]}],
            **bev_common_desc,
        },
        {
            "task": "bev_3d_pedestrian",
            "output_name": "bev_3d_pedestrian_loc_z_output",
