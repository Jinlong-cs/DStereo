...
def get_model(mode):
    ...
    roi_model = dict(
        ...
        loss=dict(
            ...
            dim_loss=dict(type="L1Loss", loss_weight=2.0, reduction="mean"),
            iou_loss_scale=1.0,
            rot_weight=6.0,
            undistort_depth_uv=undistort_depth_uv,
...
def val_update_metric_func(metrics, batch, model_outs):
    ...
    preds_parents, preds_3d = [], []
    for pred in preds:
        preds_parents.append(pred.vehicle_detection)  # noqa B009
        preds_3d.append(getattr(pred, task_name))

    predictions = {"detection": preds_parents, "det_3d": preds_3d}
...
transforms = [
    ...
    dict(
        ...
        keep_meta_keys=(
            ["uv_map", "im_hw"] if with_cam_standiardization else ["im_hw"]
        ),
    ),
]
...
val_data_loader = deepcopy(data_loader)
val_data_loader["dataset"]["shuffle"] = False
for dataset in val_data_loader["dataset"]["datasets"]:
    for sub_dataset in dataset["datasets"]:
        sub_dataset["transforms"][-1]["is_train"] = False
        sub_dataset["transforms"][-1]["keep_meta_keys"] = [
            "image_key",
            "id",
            "ignore_mask",
            "trans_matrix",
