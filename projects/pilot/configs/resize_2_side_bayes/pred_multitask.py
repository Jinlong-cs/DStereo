...
device_ids = [0]
march = "bayes"
save_res_to_vcs = ("niofy" in model_setting.lower()) and (
    not with_cam_standiardization
)
...
data_loader = dict(
    ...
    dataset=dict(
        type="YUVFrames",
        img_dir=f"{HAT_BUCKET_PATH}/users/xinjie.wang/consistency/v2/{model_setting.lower()}/resize_2_side/yuv_img",
        calib_path=f"{HAT_BUCKET_PATH}/users/xinjie.wang/consistency/v2/{model_setting.lower()}/resize_2_side/camera_0.json",
        im_hw=input_hw,
...
if is_with_pe and not with_cam_standiardization:
    ...
    if "niofy" in model_setting.lower():
        ...
        uvmap_software = load_uvmap(
            f"{HAT_BUCKET_PATH}/users/xinjie.wang/consistency/v2/{model_setting.lower()}/resize_2_side/uvmap/map_x_float_2.000000.dat",
            f"{HAT_BUCKET_PATH}/users/xinjie.wang/consistency/v2/{model_setting.lower()}/resize_2_side/uvmap/map_y_float_2.000000.dat",
            input_hw[0],
            input_hw[1],
        )
        uvmap_software_invert = load_uvmap(
            f"{HAT_BUCKET_PATH}/users/xinjie.wang/consistency/v2/{model_setting.lower()}/resize_2_side/uvmap_invert/map_x_float_h80_w120.dat",
            f"{HAT_BUCKET_PATH}/users/xinjie.wang/consistency/v2/{model_setting.lower()}/resize_2_side/uvmap_invert/map_y_float_h80_w120.dat",
            input_hw[0] // 8,
            input_hw[1] // 8,
        )
    transforms = [
