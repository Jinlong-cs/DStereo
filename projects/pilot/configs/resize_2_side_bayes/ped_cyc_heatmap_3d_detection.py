...
def get_model(mode):
    model = dict(
        ...
        postprocess=dict(
            ...
            focal_length_default=focal_length_default,
            center=np.array(list(input_hw)[::-1]),
            scale_wh=(0.5, 0.5),
            undistort_depth_uv=undistort_depth_uv,
            image_hw=input_hw,
            down_stride=down_stride,
...
inputs = dict(
    train=dict(
        heatmap=torch.zeros(
            (1, 1, resize_hw[0] // down_stride, resize_hw[1] // down_stride)
        ),
        box2d_wh=torch.zeros(
            (1, 2, resize_hw[0] // down_stride, resize_hw[1] // down_stride)
        ),
        dimensions=torch.zeros(
            (1, 3, resize_hw[0] // down_stride, resize_hw[1] // down_stride)
        ),
