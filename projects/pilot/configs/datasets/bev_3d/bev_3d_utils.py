...
def update_multi_item_in_dict(base_dict, update_dict_list: List[dict]):
    for update_dict in update_dict_list:
        base_dict = update_item_in_dict(base_dict, update_dict)
    return base_dict
    ...
    for location_name in data_dict.keys():
        for plat_name in data_dict[location_name]:
            for dataset_name, dataset_info in data_dict[location_name][
                ...
                if dataset_info is None:
                    dataset_info = {}
                if "sample_interval" not in dataset_info:
                    dataset_info["sample_interval"] = 1
...
def remove_item_in_dict(dataset_dict, required_key):
    """
    Remove item in dataset_dict based on required_key..
    Args:
        dataset_dict: the dataset needed to remove items.
        required_key: data_names that 'not' contain the required_key
    ...
    for site, plate_dict in dataset_dict.items():
        for plate, date_dict in plate_dict.items():
            for data_name in date_dict.keys():
                if required_key not in data_name:
