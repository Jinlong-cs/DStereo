...
# model info
model_type = "pilot_multitask_resize2"
...
# dataset
dataset_fname = model_setting.lower()
ds_path_template = os.path.join(
    os.path.dirname(__file__),
    "../datasets/{}_datasets.py",
)

if os.path.exists(ds_path_template.format(dataset_fname + "_rear")):
    ds_path = ds_path_template.format(dataset_fname + "_rear")
    ds_cfg = Config.fromfile(ds_path)
    datapaths = ds_cfg.datapaths
elif os.path.exists(ds_path_template.format(dataset_fname)):
    ds_path = ds_path_template.format(dataset_fname)
    ds_cfg = Config.fromfile(ds_path)
    datapaths = ds_cfg.datapaths
else:
    ds_cfg = None
...
if is_local_train:
    ...
    batch_size = 10 * batch_size_factor
    log_freq = 25
...
if ds_cfg is not None and datapaths is not None and lmdb_data:
    ...
    datapaths = parse_by_partition(
        ...
        partitions,
        training_step,
        os.path.join(ckpt_dir, "logs"),
    )
# clipping: end

input_hw = resize_hw = (
    (640, 960) if "x3c" in model_setting.lower() else (640, 1024)
)
