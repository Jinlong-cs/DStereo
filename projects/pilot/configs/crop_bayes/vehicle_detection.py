...
roi_args = dict(
    ...
    score_threshold_per_class=[
        model_thresh["det_thresh"][object_type] if model_thresh else 0.35
    ],
    nms_threshold=[0.5],
)
...
anchor_generator = dict(
    ...
    node_name=f"{object_type}_anchor",
)
...
anchor_head = dict(
    ...
    factor=2,
    group_base=16,
    node_name=f"{object_type}_anchor_head",
)
...
anchor_pred = dict(
    ...
    bbox_min_hw=(1, 1),
    node_name=f"{object_type}_anchor_pred",
)
...
anchor_desc = dict(
    ...
    node_name=f"{object_type}_anchor_desc",
)
...
roi_feat_extractor = dict(
    ...
    canonical_level=5,
    aligned=None,
    node_name=f"{object_type}_roi_feat_extractor",
)
...
roi_head = dict(
    ...
    upscale=False,
    node_name="vehicle_roi_share_head",
)
...
def get_model(mode):
    ...
    return dict(
        ...
        rpn_module=dict(
            ...
            target=dict(
                node_name=f"{object_type}_anchor_target",
            ...
            loss=dict(
                ...
                node_name=f"{object_type}_anchor_loss",
            )
            if mode == "train"
            else None,
