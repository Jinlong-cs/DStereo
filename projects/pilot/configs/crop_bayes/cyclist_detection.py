...
anchor_args = dict(
    ...
    anchor_wh_groups=[
        [[12, 13], [14, 26], [15, 37], [21, 43], [30, 63], [42, 49]],
        [[22, 52], [32, 69], [45, 91], [48, 20], [65, 68], [66, 136]],
        [
            [99, 99],
            [102, 180],
            [119, 164],
...
anchor_pred = dict(
    ...
    nms_margin=0.0,
    box_filter_threshold=model_thresh["rpn_thresh"][object_type]
    if model_thresh
    else 0.0,
    pre_nms_top_k=2000,
    ...
    per_tensor_desc=[
        json.dumps(
            dict(
                ...
                roi_input=dict(
                    fp_x=vanishing_point[0] - roi_region[0],
                    fp_y=vanishing_point[1] - roi_region[1],
                    width=roi_region[2] - roi_region[0],
                    height=roi_region[3] - roi_region[1],
                ),
                nms_threshold=0.7,
                ...
                output_name="rpn",
                score_threshold=[
                    model_thresh["rpn_thresh"][object_type]
                    if model_thresh
                    else 0.0,
                ],
            )
...
roi_feat_extractor = dict(
    ...
    canonical_level=5,
    aligned=None,
    node_name="cyclist_roi_feat_extractor",
)

roi_head = dict(
    type="RCNNMixVarGEShareHead",
    bias=True,
    bn_kwargs=bn_kwargs,
    head_config=[
        MixVarGENetConfig(
    ...
    upscale=False,
    node_name="cyclist_roi_share_head",
)
...
def get_model(mode):
    ...
    return dict(
        ...
        if not fix_downscale_neck
        else dict(
            type="ExtSequential",
            modules=[fpn_neck, fix_channel_neck, fix_downscale_neck],
        ),
        rpn_out_keys=get_det_rpn_out_keys(mode),
        rpn_module=dict(
            type="AnchorModule",
            ...
            target=dict(
                node_name="cyclist_anchor_target",
            ...
            head=dict(
                ...
                modules=[
                    ...
                    dict(
                        ...
                        reg_channel_base=4,
                        use_bin=False,
                        node_name=f"{task_name}_roi_split_head",
                    ),
                ]
                + (
                    [
                        dict(
                            ...
                            per_tensor_desc=frcnn_det_desc(
                                ...
                                score_threshold_per_class=[
                                    model_thresh["det_thresh"][object_type]
                                    if model_thresh
                                    else 0.8
                                ],
                                nms_threshold=[0.5],
