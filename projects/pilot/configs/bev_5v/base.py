        ...
        if len(added_info) > 0:
            for _info in added_info:
                ...
                if _info in ["calib_path", "homo_path", "homo_noise"]:
                    homo_gen_names = ["homo_gen", "homo_gen_small"]
                    if do_val_visualize:
                        homo_gen_names.append("homo_gen_high_sp")
                    for homo_gen_name in homo_gen_names:
                        if (
                            homo_gen_name in dataset
                            and dataset[homo_gen_name] is not None
...
def get_dataset_list(
    ...
    for dataset_dict_item in dataset_dict:
        ret_datasets = get_datasets(
            ...
            global_sample_interval=global_sample_interval,
        )
...
def get_template_dataset(
    ...
    template_dataset["homo_gen"] = homo_gen
...
def convert_to_temporal_split_dataloader(
    ...
    dataloader["dataset"]["type"] = "RankSplitE2EDataset"
    ...
    dataloader["type"] = "RankSplitE2EDataLoader"
...
def internal_state_dict_converter(state_dict):
    ...
    key_mapping = [
        ("bev_fusion_small.", "bev_fusion."),
        ("fusion_upsampling_small.", "bev_fusion_upsampling."),
    ]
...
def get_high_sp_mask(vcs_range, ego_range, sp):
    # 5v
    res = []
    camera_front = int((-vcs_range[0] + ego_range[2]) / sp)
    res.append((0, 0, 0, camera_front))

    fisheye_front = int((-vcs_range[0] + ego_range[2]) / sp)
    res.append((0, 0, 0, fisheye_front))

    fisheye_rear = int((vcs_range[2] - ego_range[0]) / sp)
    res.append((0, 0, fisheye_rear, 0))

    fisheye_left = int((-vcs_range[1] + ego_range[3]) / sp)
    res.append((0, fisheye_left, 0, 0))

    fisheye_right = int((vcs_range[3] - ego_range[1]) / sp)
    res.append((fisheye_right, 0, 0, 0))
    return res
