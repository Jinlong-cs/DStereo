...
parkingrod_metric_param = dict(
    ...
    threshold_parkingrod_feature=model_thresh.get(task_name, {}).get(
...
bev_discobj_transforms = [
    ...
    val_transforms if do_val_visualize else None,
...
def get_val_dataloader():
    ...
    val_dataset_info = get_data_dict(
        url=os.path.join(dataset_dir, f"{dataset_name}_val_version.yaml")
    )
    val_dataset_dict = {
...
def get_model(mode):
    ...
    if mode == "train":
        bev_psd_stage2_out_module["loss"] = dict(
            ...
            local_loss=dict(
                ...
                offset_loss=dict(
                    type="SmoothL1Loss", reduction="none", beta=1.0
                ),
        ...
        bev_parkingrod_stage2_out_module["convert_to_dict"] = False
        bev_parkingrod_stage2_out_module["head_parser"] = None
        bev_parkingrod_stage2_out_module["target"] = None
        bev_parkingrod_stage2_out_module["head"]["aux_branch"] = False
    ...
    model = dict(
        ...
        bev_fusion_module=dict(
            ...
            neck=bev_neck,
            head=bev_parkingrod_stage2_out_module,
        ),
...
#         output_dir=os.path.join(save_prefix, "visualize", "bev_parkingrod"),
#         task="bev_parkingrod",
#         prefix="bev_stage2_psd_small_head_predict",
#         score_threshold=0.2,
#         vis_img_scale=1,
#         bev_size=ipm_output_size,
#         vcs_range=vcs_range,
...
#         camera_layouts=None,
#         res_key2anno_name={
#             "bev_parkingrod": "annos_bev_discrete_obj",
#         },
#         color_map=COLOR_MAP,
#         id2label=ID2LABEL,
#         visual_interval=1,
...
vis_small = update_content(
    ...
    res_key2anno_name={
        "bev_parkingrod": "annos_bev_parkingrod_obj",
    },
    project_pts_to_cameras=True,  # if do_pack_infer else True,
    task="parkingrod",
    output_dir=os.path.join(save_prefix, "visualize", "bev_parkingrod"),
    prefix="bev_stage2_parkingrod_small_head_predict_",
    reformat_fn=lambda x: x["bev_parkingrod"][0]
    if type(x["bev_parkingrod"]) is list
    else x["bev_parkingrod"],
)
visualize_callbacks = [vis_small]

# -------------------------- PACK_INFER_SAVE --------------------------
save_pack_infer = dict(
    type="SaveParkingRodConsistencyResult",
    output_dir=os.path.join(save_prefix, "inference", "bev_parkingrod"),
    output_key="bev_stage2_parkingrod_small_head_predict_preds_parkingrod",
    vcs_range=rod_roi_vcs_range,
    bev_parkingrod_gt_size=bev_parkingrod_gt_size,
)
