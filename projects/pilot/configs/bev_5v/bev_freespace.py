...
# ------------------ DATASET SETTING -------------------
# train data
train_data_version = "v5_2_0_fisheye"
val_data_version = [
    "v3.7.1_all_parking",
    "v2.1.3_small_obstacle_11v",
    "v1.0.0_ek_parking_underground",
    # "v1.0.0_ek_parking_ground",
    # "v1.0.0_parking_overground_rough",
    # "v1.0.0_parking_underground_rough",
]
if pipeline_test:
...
collect_3dv["load_data_types"] = [f"gt_{task_name}", "timestamp", "img_name"]
...
template_dataset = get_template_dataset(
    ...
    transforms=bev_freespace_transforms,
    homo_transforms=get_homo_transforms(
        transforms_list=bev_freespace_transforms,
        camera_view_names=camera_view_names,
    ),
...
def get_model(mode):
    ...
    bev_head = dict(
        ...
        postprocess=None,
        target=None,
        node_name="bev_stage2_freespace_small_head",
    )
...
# ------------------------- VALIDATION SETTING-----------------------
# ego range: [-1.2, -1.1, 4.1, 1.1]
# < 3m vcs_range: [-4.3, -4.1, 7.1, 4.1]
# < 5m vcs_range: [-6.2, -6.1, 9.1, 6.1]
eval_setting = {
    "eval_vcs_range": None,
    ...
    "depth_intervals": (-12.8, 0, 10, 20, 30),
...
def tb_update_func(writer, model_outs, global_step_id, **kwargs):
    ...
    target_regex = re.compile(f"^.*target_gt_{task_name}")
    if task_name in list(model_outs.keys())[0]:
        for k, v in model_outs.items():
            if pred_regex.match(k) or target_regex.match(k):
                data_vis = v.detach().cpu().numpy()[0, :, :]
                writer.add_images(
                    k, data_vis[:, :, None], global_step_id, dataformats="HWC"
                )
