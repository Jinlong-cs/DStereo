...
# -------------------------- data --------------------------
train_data_version = "pilot_v1_0_0_small"
val_data_version = "pilot_v1_0_0_small"
...
name2label = {"intersections": "Junction"}
...
bev_discobj_target = dict(
    ...
    ignore_miss_cls=ignore_miss_cls,
    # if merge crosswalk to junction, all junction data
    # contains crosswalk label, but some crosswalk data
    # miss junction label, it is necessary to ignore
    # the data which miss junction label
    ignore_miss_clsid=0 if ignore_miss_cls else None,
...
def get_train_dataloader():
    ...
    if use_split_dataloader:
        data_loader = convert_to_split_dataloader(
            data_loader, dataloader_seed=0, temporal_bev=False
        )
    return data_loader
...
def get_model(mode):
    bev_head = dict(
        ...
        postprocess=None,
        target=None,
        node_name="bev_stage2_junction_small_head",
    )
    if mode == "train":
        bev_head["loss"] = dict(
