...
def get_model(mode):
    ...
    if mode == "train":
        ...
        bev_psd_stage2_out_module["convert_to_dict"] = False
        bev_psd_stage2_out_module["head_parser"] = None
        bev_psd_stage2_out_module["target"] = None
    ...
    model = dict(
        ...
        bev_fusion_module=dict(
            ...
            neck=bev_neck,
            head=bev_psd_stage2_out_module,
        ),
...
#         output_dir=os.path.join(save_prefix, "visualize", "bev_psd"),
#         task="bev_psd",
...
#         camera_layouts=None,
#         res_key2anno_name={
#             "bev_psd": "annos_bev_discrete_obj",
#         },
#         color_map=COLOR_MAP,
#         id2label=ID2LABEL,
#         visual_interval=1,
...
vis_small = update_content(
    ...
    task="psd",
    output_dir=os.path.join(save_prefix, "visualize", "bev_psd"),
    prefix="bev_stage2_psd_small_head_predict_",
    reformat_fn=lambda x: x["bev_psd"][0]
    if type(x["bev_psd"]) is list
    else x["bev_psd"],
)
...
save_pack_infer = dict(
    type="SavePsdConsistencyResult",
    output_dir=os.path.join(save_prefix, "inference", "bev_psd"),
    output_key="bev_stage2_psd_small_head_predict_decode_label",
    vcs_range=psd_roi_vcs_range,
    bev_psd_gt_size=bev_psd_gt_size,
)
