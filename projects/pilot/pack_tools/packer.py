...
class LmdbDefaultPacker(object):
    ...
    def pack_img(self):
        ...
        logger.info("Begin packing...")
        self.data_packer.pack_img()

    def pack_anno(self):
        verborse_list = []
        logger.info("~~~Packing ts anno for task %s~~~" % self.cfg.task_name)
        ...
        if self._verbose:
            for name, cost_time in verborse_list:
                logger.info(name + str(cost_time))

    def visualize(self):
        logger.info("Visualizing task %s" % self.cfg.task_name)
