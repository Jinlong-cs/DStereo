...
model_type = "t8"
task_name = f"fastvit-qa-{model_type}_cls"
num_classes = 1000
...
val_data_loader = dict(
    ...
    dataset=dict(
        ...
        out_pil=True,
        transforms=[
            dict(type="TorchVisionAdapter", interface="Resize", size=280),
            dict(type="TorchVisionAdapter", interface="CenterCrop", size=256),
...
float_trainer = dict(
    ...
    optimizer=dict(type=torch.optim.AdamW, lr=0.001, weight_decay=0.05),
    ...
    model_convert_pipeline=dict(
        ...
        qconfig_params=dict(
            activation_calibration_qkwargs=dict(
                update_interval=calibration_step,
            ),
            activation_calibration_observer="kl",
        ),
        converters=[
            dict(
                type="LoadCheckpoint",
...
qat_trainer = dict(
    ...
    optimizer=dict(
        ...
        params={"weight": dict(weight_decay=5e-2)},
        lr=0.0001,
    ),
    batch_processor=batch_processor,
    num_epochs=100,
...
infer_transforms = Compose(build_from_registry(infer_transforms))
