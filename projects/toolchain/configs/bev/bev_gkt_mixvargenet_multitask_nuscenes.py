...
def get_input_point(num_points, use_placeholder=True):
    inputs = {"img": torch.randn((6,) + data_shape)}
    for i in range(num_points):
        if use_placeholder is True:
            inputs[f"points{i}"] = placeholder(
                (6, 64, 64, 2),
                torch_native=True,
                sample=torch.rand((6, 64, 64, 2)),
            )
        else:
...
deploy_inputs = get_input_point(9, True)
# data
data_loader = dict(
...
onnx_cfg = dict(
    model=deploy_model,
    inputs=get_input_point(9, False),
    stage="qat",
    model_convert_pipeline=dict(
        type="ModelConvertPipeline",
        qat_mode="fuse_bn",
