...
def get_input_point(num_points, use_placeholder=True):
    ...
    if use_placeholder is True:
        inputs["points"] = placeholder(
            (num_points, H, W, 2),
            torch_native=True,
            sample=torch.rand((num_points, H, W, 2)),
        )

        inputs["points_depth"] = placeholder(
            (num_points, H, W, 2),
            torch_native=True,
            sample=torch.rand((num_points, H, W, 2)),
        )
    else:
        inputs["points"] = torch.randn(
            (num_points, H, W, 2),
        )

        inputs["points_depth"] = torch.randn(
            (num_points, H, W, 2),
        )
...
deploy_inputs = get_input_point(num_points, True)
...
onnx_cfg = dict(
    model=deploy_model,
    inputs=get_input_point(num_points, False),
    stage="qat",
    model_convert_pipeline=dict(
        type="ModelConvertPipeline",
        qat_mode="fuse_bn",
