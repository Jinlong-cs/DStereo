...
model = dict(
    ...
    neck=dict(
        ...
        stride2channels=dict(
            {2: 32, 4: 32, 8: 64, 16: 96, 32: 160, 64: 160, 128: 160, 256: 160}
        ),
        out_channels=dict({4: 32, 8: 160, 16: 160, 32: 160, 64: 256}),
        num_outs=5,
        stack=1,
        start_level=1,
        end_level=-1,
...
def get_input_point(num_points):
    ...
    for i in range(num_points):
        inputs[f"points{i}"] = torch.rand((6, 64, 64, 2))

    return inputs


deploy_inputs = get_input_point(9)
# data
data_loader = dict(
    ...
    model_convert_pipeline=dict(
        ...
        converters=[
            dict(
                type="LoadCheckpoint",
                checkpoint_path=(
                    "./tmp_pretrained_models/mixvargenet_imagenet/float-checkpoint-best.pth.tar"  # noqa: E501
                ),
                allow_miss=True,
                ignore_extra=True,
            ),
...
compile_cfg = dict(
    ...
    layer_details=True,
    input_source="pyramid,ddr,ddr,ddr,ddr,ddr,ddr,ddr,ddr,ddr",
    opt="O3",
    transpose_dim=dict(
        inputs={
            "global": [0, 2, 3, 1],
...
def process_inputs(infer_inputs, transforms=None):
    ...
    inputs_dirs = list(os.listdir(infer_inputs["imagedir"]))
    inputs_dirs = sorted(inputs_dirs)
    for i, img in enumerate(inputs_dirs):
