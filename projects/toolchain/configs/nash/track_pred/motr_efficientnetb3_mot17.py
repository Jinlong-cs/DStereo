...
hbir_infer_model = dict(
    ...
    qim_model_path=os.path.join(ckpt_dir, "qim", "qat.bc"),
    post_process=dict(
        type="MotrPostProcess",
    ),
    LoadCheckpoint=dict(
        ...
        "frame_data_list": [
            {
                ...
                "layout": "hwc",
                "color_space": "rgb",
                "img_name": ["/".join(infer_inputs.split("/")[-3:])],
                "seq_name": [infer_inputs.split("/")[-3]],
            },
        ]
    }
...
def prepare_inputs(infer_inputs):
    inputs_imgs = sorted(
        glob.glob(os.path.join(infer_inputs["imagedir"], "*.jpg"))
    )
    if "num" in infer_inputs:
        return inputs_imgs[: int(infer_inputs["num"])]
    return inputs_imgs
...
infer_cfg = dict(
    model=hbir_infer_model,
    infer_inputs=dict(
        imagedir="./tmp_orig_data/mot17/split_data/test/MOT17-04-SDP/img1",
        num="2",
    ),
