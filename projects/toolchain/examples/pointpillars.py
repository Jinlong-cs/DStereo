...
from hat.models.structures.detectors.pointpillars import PointPillarsDetector
...
def predict_entrance(
    ...
    if stage == "float":
        ...
        model = PointPillarsModel.int_model(pretrain_ckpt=ckpt)
    else:
        raise ValueError(
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        default="ddr",
        help="input source for compile, multi-values splited by ','`",
    )
    parser.add_argument(
        "--output-layout",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=horizon.march.March.BAYES,
        help="march of bpu",
    )
...
def main():
    ...
    if args.train:
        ...
        if stage == "float":
            ...
            model = PointPillarsModel.int_model(
                pre_step_ckpt=pre_step_ckpt,
                use_deploy=True,
            )
    ...
    if args.check_model:
        model_checker(
            deploy_model=PointPillarsModel.int_model(use_deploy=True),
            deploy_inputs=PointPillarsModel.deploy_inputs(),
        )
    ...
    if args.compile:
        pre_step_ckpt = os.path.join(ckpt_dir, "qat-checkpoint-best.pth.tar")
        int_deploy_model = PointPillarsModel.int_model(
            pre_step_ckpt=pre_step_ckpt, use_deploy=True
        )
        input_source = args.input_source.split(",")
        compile(
            int_deploy_model,
            ...
            int_ckpt=args.ckpt,
            opt=args.opt,
            input_source=input_source,  # noqa B006
