...
def parse_args():
    ...
    parser.add_argument(
        "--input-image", type=str, required=True, help="Input file."
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help="Crop image with fixed position x, y, w, h",
    )
...
if __name__ == "__main__":
    ...
    orig_img = cv2.imread(args.input_image)
    ...
    fixed_crop_offset = [0, 0, orig_img.shape[-1], orig_img.shape[-2]]
    if args.fixed_crop is not None:
        fixed_crop_offset = list(map(int, args.fixed_crop.split(",")))
    fixed_img = orig_img[
        :,
        fixed_crop_offset[1] : fixed_crop_offset[1] + fixed_crop_offset[3],
        fixed_crop_offset[0] : fixed_crop_offset[0] + fixed_crop_offset[2],
    ]
    resized_img = resize(fixed_img, input_size[:2]).unsqueeze(0)
    ...
    h_scale = input_img.shape[-2] / fixed_img.shape[-2]
    w_scale = input_img.shape[-1] / fixed_img.shape[-1]
    scale_factor = [
        torch.tensor(
