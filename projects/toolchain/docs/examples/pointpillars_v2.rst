...
不做详细介绍。我们通过定义一个 `PointPillarsModel` 类来与模型结构相关的内容：
...
至此， `PointPillarsModel` 中已定义了与模型相关的所有内容，在使用时可以很方便的通过 `PointPillarsModel.xxx()` 获取相应模块。


数据增强
>>>>>>>>>

类似`model`部分，我们通过定义一个 `DataHelper` 来实现数据相关内容，包括 `transforms` ， `data_loader` 等：
...
通过上面的介绍，我们已经完成了对模型训练相关所有模块的定义，接下来就可以训练一个高精度的纯浮点的检测模型。当然训练一个\
...
只需要相应的运行以下两条命令即可：
  ...
  .. code-block:: python
    ...
    python3 examples/pointpillars.py --predict --stage "float" --device-ids "0" --ckpt ${float-checkpoint-file}
    python3 examples/pointpillars.py --predict --stage "qat" --device-ids "0" --ckpt ${qat-checkpoint-file}
