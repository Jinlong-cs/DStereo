...
task_name = f"{object_type}_pose_{task_type}"

data_args = dict(legacy_bbox=True)

sub_classnames = ["Bended", "Cyclist", "Lier", "Pedestrian", "Sitter"]
...
# ------------------------ dataloader ---------------------------
# data
ds = datapaths.person_pose_classification
...
(
    ...
    old_prediction,
    new_prediction,
) = get_rcnn_aidi_eval_info("person_pose_classification", "person")

aidi_eval_loader = get_2d_aidi_eval_loaders(
    batch_size["val"], "person_pose_classification", infer_model_type="rcnn"
)
