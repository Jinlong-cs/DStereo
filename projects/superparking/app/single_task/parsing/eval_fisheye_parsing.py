...
cfg = Config.fromfile(this_dir + "/fisheye_parsing.py")
if_aidi_eval = True
...
for i in dataset_id_list:
    aidi_eval_callback = dict(
        ...
        reformat_out_fn_kwargs={"obj_key": "semseg_pred"},
    )
    aidi_eval_callbacks.append(aidi_eval_callback)
aidi_val_data_loader = []
for i in dataset_val_img_path:
    ...
    aidi_val_data_loader.append(val_data_loader_i)

# For your information: need to turn off relu6 check for this checkpoint file
qat_pred = "dmpv2://SuperParking/yunfeng.zhang/model_tmp/fisheye_parsing/v0.9.0/qat-checkpoint-best-0242e310.pth.tar"
qat_pred = bkt_clt.url_to_local(qat_pred)
val_metric_updater = cfg.val_metric_updater
rec_val_data_loader = cfg.val_data_loader
if if_aidi_eval:
    eval_callbacks = aidi_eval_callbacks
    val_data_loader = aidi_val_data_loader
else:
    eval_callbacks = [val_metric_updater]
    val_data_loader = rec_val_data_loader
int_predictor = dict(
    type="Predictor",
    model=val_model,
