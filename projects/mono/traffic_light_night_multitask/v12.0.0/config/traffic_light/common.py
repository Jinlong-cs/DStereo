...
# model info
model_type = "traffic_light_night_multitask"
model_name = "_".join([model_type, model_setting])
compile_model = False
march = March.BERNOULLI2
mask_in_bpu = False if march == March.BERNOULLI2 else True
deploy_batch = 1
...
cls_pooling_padding = 0 if input_size[0] == 96 else 1
...
def get_stride2channels():
    pc = [int(c * backbone.get("alpha", 1)) for c in backbone["channel_list"]]
...
val_transforms = [
    ...
    dict(
        ...
        input_wh=input_size,
        mask_out_gt_crop_flag=False if march == March.BERNOULLI2 else True,
        extend_ratio=0.1,
        mask_in_bpu=mask_in_bpu,
    ),
    dict(
