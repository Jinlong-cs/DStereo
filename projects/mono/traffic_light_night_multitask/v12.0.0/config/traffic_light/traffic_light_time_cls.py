...
classnames = ["traffic_light_time"]
object_type = "_".join(classnames)
task_name = f"{object_type}_{task_type}"
traffic_light_time_sub_classnames = [str(_) for _ in range(1000)]

# model
num_classes = len(traffic_light_time_sub_classnames)
...
def get_model(mode):
    model = dict(
        ...
        losses=dict(
            type="TrafficTimeCrossEntropyLoss",
...
inputs = dict(
    train=dict(
        ...
        multi_label=torch.zeros((1, 4)).long(),
    ),
    val=dict(),
    test=dict(),
)
...
metric_updater = dict(
    type="MetricUpdater",
    metrics=[
        dict(type="LossShow", name="nums_digit_loss"),
        dict(type="LossShow", name="ones_place_loss"),
        dict(type="LossShow", name="tens_place_loss"),
        dict(type="LossShow", name="hundreds_place_loss"),
    ],
    metric_update_func=update_metric_using_regex(
        per_metric_patterns=[  # corresponding to metrics
            dict(
                label_pattern=None,
                pred_pattern=f"^.*{task_name}_nums_digit_loss$",
            ),
            dict(
                label_pattern=None,
                pred_pattern=f"^.*{task_name}_ones_place_loss$",
            ),
            dict(
                label_pattern=None,
                pred_pattern=f"^.*{task_name}_tens_place_loss$",
            ),
            dict(
                label_pattern=None,
                pred_pattern=f"^.*{task_name}_hundreds_place_loss$",
            ),
        ]
    ),
...
ds = datapaths.traffic_light_time
            ...
            dict(
                ...
                transforms=[
                    ...
                    dict(
                        ...
                        input_wh=input_size,
                        mask_out_gt_crop_flag=mask_in_bpu,
                        extend_ratio=0.1,
                        mask_in_bpu=mask_in_bpu,
                    ),
                    dict(
