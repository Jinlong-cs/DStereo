...
# ---------------------------------- multitask ----------------------
job_name = "PersonCyclistDet2PE-v2.0.0.0"
...
def get_per_task_flat_condition(mode, task_configs):
    condition_dict = {}
    assert mode in ["train", "val", "test", "deploy"]
    if mode == "train":
...
deploy_model = get_task_model("deploy", TASK_CONFIGS)
...
int_infer_trainer = dict(
    type="Trainer",
    model=get_task_model("deploy", TASK_CONFIGS),
    model_convert_pipeline=dict(
        type="ModelConvertPipeline",
        qat_mode="fuse_bn",
