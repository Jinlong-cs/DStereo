...
remote_pipeline_test = False
pipeline_test = False or remote_pipeline_test
...
base_step = (
    100
    if remote_pipeline_test
    else 10
    if pipeline_test
    else 50000 // math.sqrt(batch_size_factor)
)
# trainer (for 1 machine 8 gpus case)
lr = 0.001 * math.sqrt(batch_size_factor)
freeze_bn_lr = 0.0003
qat_lr = 0.00005
warmup_begin_lr = 0.000001
wd = 1e-05
log_freq = int(base_step / 100 + 1)
float_steps = int(base_step + 1)
freeze_bn_steps = int(base_step / 2 + 1)
qat_steps = int(base_step / 4 + 1)
warmup_steps = int(base_step / 50 + 1)
save_interval = int(base_step / 5 + 1)
interval_by = "step"
