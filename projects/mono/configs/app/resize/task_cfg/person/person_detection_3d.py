...
visual_save_path = f"{visual_save_base}/Person"
...
data_paths = get_data_path(
    bucket_root, f"{dataset_root}/{camera_type}/person_filtered.yaml"
)
train_rec_paths = []
val_rec_paths = []
...
val_metric_updater = get_val_metric_updater(
    ...
    save_name="person.xlsx",
)

# -----------------------json for aidi eval-----------------------------
# 每一次aidi评测的名字，确保跟已上传的名字不同，便于对比不同的评测
# 格式：版本 + 模型特点 + 时间
prediction_name = "new_config_real3d_vehicle"
aidi_eval_callback = dict(
    type="AIDIEval",
    output_root=visual_save_path,
    project_id=project_id,
    prediction_name=prediction_name,
    aidi_eval_dataset_id=[6037298],
...
if aidi_eval:
    val_metric_updater = aidi_eval_callback
