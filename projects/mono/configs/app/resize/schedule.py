...
from common import is_int_infer

add_2nd_stage = False  # currently 2nd stage not supported
tasks_1st_stage = dict(
    ...
    person_tasks=[
        ...
        # dict(name="person_detection_3d", important=True, compile=False), # not ready
    ],
    cyclist_tasks=[
        dict(name="cyclist_detection", important=True),
        # dict(name="cyclist_detection_3d", important=True, compile=False), # not ready
    ],
    rear_tasks=[
        dict(name="rear_detection", important=True),
    ],
    parsing_tasks=[
        dict(name="parsing_panoptic", important=True),
        dict(name="parsing_lane", important=True),
    ],
)
...
if add_2nd_stage:
    for task_name in tasks_1st_stage:
        if task_name not in tasks_2nd_stage:
            continue
...
for t in tasks:
    ...
    if is_int_infer and t.get("compile", True) is False:
        continue
    task_names.append(task_name)

config_file = "projects/" + os.path.abspath(__file__).split("/projects/")[-1]
config_file_root = os.path.dirname(config_file).replace("/", ".")
TASK_CONFIGS = [import_module(t, config_file_root) for t in task_names]
