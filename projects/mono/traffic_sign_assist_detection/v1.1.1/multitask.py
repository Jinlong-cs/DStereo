...
# qat_resume_checkpoint = "http://fm-yanlei-zhang.tcloud.hogpu.cc/plat_gpu/hobot-dag-3599297_TrafficSignAssistDetection-v1-0-0-20230803-115607/output/models/mono_traffic_sign_assist_detection_v1.0.0/qat-checkpoint-last-5a4ee8db.pth.tar" # noqa
...
aidi_expmodel_callback = dict(
    type="AIDIExpModel",
    model_name="hat_traffic_sign_assist_testjm",
    task_type="multitask",
    save_model="last",
...
qat_converters = [
    dict(
        ...
        if qat_resume_checkpoint is None
        else None,
        allow_miss=False,
        ignore_extra=True,
        check_hash=False,
...
if not pipeline_test:
    qat_callbacks.append(aidi_expmodel_callback)
...
if not pipeline_test:
    int_callbacks.append(aidi_expmodel_callback)
...
compile_cfg = dict(
    march=march,
    name=compile_task_name,  # Name of the model, recorded in hbm
    out_dir=compile_dir,
    hbm=os.path.join(compile_dir, f"{compile_task_name}.hbm"),
    layer_details=False,
    input_source=["resizer"],  # or ddr? custom by yourself
    opt="O3",  # change O3 for faster fps
    extra_args=["--dev-remove-extra-output-cpu-op"],
)
...
job_list = [
    "python3 -m pip install numpy==1.23.5 -i https://pypi.hobot.cc/simple --extra-index-url https://pypi.hobot.cc/hobot-local/simple --trusted-host pypi.hobot.cc",  # noqa
...
k8s_config = dict(
    ...
    framework="pytorch",
    task_label="mono_traffic_sign_assist_detection_multitask",
    project_id="PDT20220004",
    input_bucket=",".join(buckets),
    priority=5,
    docker_image="docker.hobot.cc/dlp/hat:runtime-py3.8-torch1.10.2-cu111-1.4.1",  # noqa
    max_jobtime=20160,
