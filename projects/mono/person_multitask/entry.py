...
# -------------------------- exp --------------------------
job_name = (
    "hat_mono3.2_2pe_person_multitask_v13.0.3_2machines_lrmul_impv2_fixall"
)

lr = 1e-2 * num_machines
freeze_bn_lr = 2e-4 * num_machines
qat_lr = 2e-4 * num_machines
wd = 1e-2
use_step_lr = False
float_steps = 25000 // num_machines + 2000 if num_machines > 1 else 0
freeze_bn_steps = 10000 // num_machines + 1000 if num_machines > 1 else 0
qat_num_steps = 10000 // num_machines + 1000 if num_machines > 1 else 0
warmup_steps = 500
save_interval = 1000
do_freeze_bn = False
freezebn_step_ids = [0]
...
def get_topo_builder(mode, out_feat_idx=None):
    def _build_topo(nodes, inputs):
        ...
        if compile_model and mode == "test" and BASE_CONFIG.add_tracking:
            name2out.update(
                {
                    "tracking_feature": nodes["tracking_feature_head"](
                        backbone_feats
                    )
                }
            )
...
if compile_model and BASE_CONFIG.add_tracking:
    test_model["nodes"]["tracking_feature_head"] = BASE_CONFIG.tracking_module
...
qat_trainer = dict(
    ...
    data_loader=data_loader,
    optimizer=dict(type=torch.optim.AdamW, lr=qat_lr, weight_decay=wd),
    batch_processor=batch_processor,
    stop_by="step",
...
compile_cfg = dict(
    ...
    out_dir=compile_dir,
    hbm=os.path.join(compile_dir, "person_multitask.hbm"),
    layer_details=False,
...
job_list = [
    ...
    f"python3 -W ignore tools/train.py --config {cfg_file} -s qat",
    f"export EXPORT_COMPILE_MODEL=True;python3 -W ignore tools/train.py --config {cfg_file} -s int_infer",  # noqa
]
...
dockers = dict(
    ...
    # cu111="docker.hobot.cc/dlp/hat:runtime-py3.8-torch1.10.2-cu111-9144e18",
    cu111="docker.hobot.cc/dlp/hat:runtime-py3.8-torch1.10.2-cu111-1.4.1",
    cu118="docker.hobot.cc/dlp/hat:runtime-py3.8-torch2.0.1-cu118-2.0.1",
)
...
k8s_config = dict(
    ...
    framework="pytorch",
    task_label="2pe_multitask",
    project_id="PDT20220004",  # "PDT20220001",#   "PDT20220004",
    input_bucket=input_bucket,
    priority=5,
    docker_image=dockers["cu118"],
    # default 7200 = 5days
    max_jobtime=60 if num_gpus_per_machine <= 2 else 10000,
    ...
    folder_list=[
        ...
        "plugins/k8s_submit/watch_dog.py",
        "plugins/k8s_submit/elastic_launcher.py",
        "plugins/k8s_submit/default_elastic_patterns.yaml",
        "plugins/k8s_submit/torchrun",
    ],
    job_list=job_list,
    custom_cmds_before_job_list=[
        "pip3 install --user hatbc==0.10.0 -i https://pypi.hobot.cc/simple --extra-index-url=https://pypi.hobot.cc/hobot-local/simple --trusted-host pypi.hobot.cc",  # noqa
