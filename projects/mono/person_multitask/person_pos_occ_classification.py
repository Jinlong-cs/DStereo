...
task_name = f"person_pos_occ_{task_type}"
divisor = 64
num_classes = 5
...
val_data_loader = dict(
    ...
    dataset=dict(
        ...
        data_path=dataset_val_img_path[4],
        to_rgb=True,
...
neck_module = dict(
    type="ClassificationExtendHead",
    in_channel=backbone_out_channels[-2],
    o_channel=backbone_out_channels[-1],
    end_feature_idx=len(backbone_out_channels) - 1
    if 1
    else len(backbone_out_channels),
    num_units=2,
    group_base=8,
    bn_kwargs=bn_kwargs,
)
...
out_module = dict(
    ...
    head=dict(
        ...
        output_by_argmax=False if BASE_CONFIG.march == March.BAYES else True,
    ),
    target=lambda x, _: x.get("gt_classes").reshape(-1),
    loss=dict(
        type=torch.nn.CrossEntropyLoss,
        label_smoothing=0.01,
        weight=torch.tensor([4.0, 1, 1, 1, 1])
...
nodes = {f"{task_name}_neck": neck_module, f"{task_name}_head": out_module}
val_nodes = {
