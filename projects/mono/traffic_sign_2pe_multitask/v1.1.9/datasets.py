...
batch_size_map = dict(
    traffic_sign_cn_category_classification=512,
    traffic_sign_occlusion_classification=512,
)
val_batch_size = 128
    ...
    datapaths[cls_name] = dict(
        ...
        train_data_paths=[
            dict(
                ...
                roi_list_path=(
                    _get_root(i["train_roi_list"]) + i["train_roi_list"]
                )
                if ("train_roi_list" in i and i["train_roi_list"] is not None)
                else None,  # noqa
...
def parse_dataset(datapaths, take_classnames, check_path=True):
    ...
    for name_i in _as_list(take_classnames):
        ...
        def _update(src, dst):
            ...
            dst["train_data_desc"]["roi_list_paths"].extend(
                _flat(src["train_data_paths"], "roi_list_path")
            )  # noqa
            ...
            dst["val_data_desc"]["roi_list_paths"].extend(
                _flat(src["val_data_paths"], "roi_list_path")
            )  # noqa
    ...
    if check_path:
        ...
        _check(
            results["train_data_desc"]["roi_list_paths"], [".pb_rec", ".json"]
        )
        _check(
        ...
        _check(
            results["val_data_desc"]["roi_list_paths"], [".pb_rec", ".json"]
        )
