...
if is_local_train:
    ...
    qat_trainer.pop("sync_bn")
    # float_trainer["type"] = "Trainer"
    # qat_trainer["type"] = "Trainer"
...
compile_cfg = dict(
    ...
    out_dir=compile_dir,
    hbm=os.path.join(compile_dir, "traffic_sign.hbm"),
    layer_details=True,
...
requirements_file = os.path.join(config_file_root, "requirements")
job_list = [
...
k8s_config = dict(
    ...
    framework="pytorch",
    task_label="mono_traffic_sign_multitask",
    project_id="PDT20220004",
    input_bucket=",".join(buckets),
    priority=5,
    docker_image="docker.hobot.cc/dlp/hat:runtime-py3.8-torch1.10.2-cu111-1.3.2",  # noqa
    # default 7200 = 5days
    max_jobtime=20160,
    ...
    custom_cmds_before_job_list=[
        f"python3 -m pip install -r {requirements_file} -i https://pypi.hobot.cc/simple --extra-index-url https://pypi.hobot.cc/hobot-local/simple --trusted-host pypi.hobot.cc",  # noqa
        f"sh {os.path.join(config_file_root, 'show_env.sh')}",
    ],
)
