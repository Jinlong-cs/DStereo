...
def parse_full_args():
    ...
    parser.add_argument("--sub-project", type=str, default="wk")
...
def run_pipeline(
    ...
    k8s_config = f"{DIR_PATH}/../mono_wk_cluster_cfg.yaml"
    ...
    train_cmd_pattern = (
        ...
        + f" --hat-wk-model-setting {model_setting}"
        + f" --hat-num-machines {num_machines}"
    )

    if model_name_postfix is not None:
        train_cmd_pattern += (
            f" --hat-wk-model-name-postfix {model_name_postfix}"
        )
    if model_version is None:
        model_version = f"{DIR_PATH}".split("/")[-3]
    train_cmd_pattern += f" --hat-wk-model-version {model_version}"
    ...
    if local:
        ...
        if eval_online:
            eval_cmd_pattern = [
                f"export HAT_WK_PREDICTION_NAME_SUFFIX=sd_{model_version}"
            ]
            eval_cmd_pattern += [
                (
                    "python3 -W ignore tools/predict.py"
                    + " --config {DIR_PATH}/../../config/work_condition/eval_multitask.py"  # noqa
                    + " --stage freeze_bn_2 --device-ids "
                    + ",".join(list(map(str, range(num_gpus_per_machine))))
                    + " --hat-wk-model-setting wk_multitask"
                    + " --hat-num-machines 1"
                )
            ]
