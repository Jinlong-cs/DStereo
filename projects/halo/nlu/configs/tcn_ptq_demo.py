...
# 训练相关参数
task_name = "tcn_ptq_demo"
batch_size_per_gpu = 2
# 不建议多机 可能是通信带来的成本 实际多机训练时速度很慢
device_ids = [1]
learning_rate = 1e-3
base_jfs_path = "file:///jfs-hdfs/user/nlp/nlu_model/demo/data"
...
val_path = os.path.join(base_jfs_path, "dataset/trainset")
predict_path = os.path.join(base_jfs_path, "dataset/trainset")

# 部署相关参数
# 用来验证量化后的模型性能损失，一般为内部测试集的子集。
deploy_evalset_path = (
    "file:///jfs-hdfs/user/nlp/nlu_model/demo/data/dataset/trainset"
)
# 部署平台类型： j3 或 j5
platform = "j5"
hbdk_version = "3.37.1"
...
upload_path = "jfs://jfs-hdfs/user/ruosong.li/nlu_e2e_model/"
...
def update_metric(metrics, batch, model_outs):  # batch参数写死了，必须得传
