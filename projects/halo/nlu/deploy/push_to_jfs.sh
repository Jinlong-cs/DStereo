...
#Function: Push related model files(PTQ) and data to jfs.

usage="Usage: ./deploy/push_to_jfs.sh configs/tcn_nlu.cfg"
...
if [[ "${platform}" = j3 ]] || [[ "${platform}" = j5 ]]; then
    release_path=${upload_jfs_path}/ptq/release_${platform}_hbdk${hbdk_version}
else
...
cp ${deploy_save_dir}/special_layers/embedding.float.weight.bin ${to_upload_path}/embedding.weight.bin
...
if hdfs dfs -test -e "${upload_jfs_path}"; then
    ...
    if [[ ${answer} = "y" ]] || [[ ${answer} = "Y" ]]; then
        echo "NOTICE: delete old files and push new files"
        hdfs dfs -rm -r "${upload_jfs_path}"/ptq/ckpt
        hdfs dfs -rm -r "${upload_jfs_path}"/onnx
    ...
    hdfs dfs -mkdir "${upload_jfs_path}"/ptq
fi
...
hdfs dfs -mkdir "${upload_jfs_path}"/ptq/ckpt
hdfs dfs -mkdir "${upload_jfs_path}"/onnx
hdfs dfs -mkdir "${release_path}"

echo "NOTICE: pushing files"
# ckpt推送
ckpt_file=output/${task_name}//float-checkpoint-best.pth.tar
hdfs dfs -put "${ckpt_file}" "${upload_jfs_path}"/ptq/ckpt
...
hdfs dfs -put "${deploy_save_dir}"/${platform}/hat-tcn.bin "${release_path}"/base_best.bin
