...
from hat.data.transforms.avspeech.denoise import OnLineDenoisePipeline
...
model_prefix = "baseline3_strcture_v2_bucket_sampler_d8_b8_add_j2mm_1000id_image_aug_audio_aug_snr0-20_local_rir"
...
if running_in_cluster():
    ...
    num_worker = 6
    prefetch_factor = 4
    gpu_rir_path = "/bucket/input/J2MM/speech/hdf5/gpu_rirs/gpu_rirs.hdf5"
    train_hdf5_audio_path = (
        "/bucket/input/J2MM/speech/hdf5/audio.hdf5"  # cluster
    )
    val_hdf5_audio_path = {
        "FYZ": "/bucket/input/J2MM/speech/hdf5/mmasr_val/FYZ_val_mixed_no_hisf",
        "QR": "/bucket/input/J2MM/speech/hdf5/mmasr_val/QR_val_mixed_no_hisf",
    }
    test_hdf5_audio_path = (
        "/bucket/input/J2MM/speech/hdf5/mmasr_test/QR_test_mixed_no_hisf"
    )
    noise_hdf5_file = "/bucket/input/J2MM/speech/hdf5/noise.hdf5"
    ckpt_dir = f"/job_data/models/{task_name}/"
    epoch_log_freq = 1
    step_log_freq = 100
    train_info_txt = "/bucket/input/J2MM/speech/hdf5/mmasr_info/train/train_fyz_j2mm_1000id.info"
    val_info_txt = {
        "FYZ": "/bucket/input/J2MM/speech/hdf5/mmasr_info/val/val.info",
        "QR": "/bucket/input/J2MM/speech/hdf5/mmasr_info/val/qr_choose_500_for_val.info",
    }
    test_info_txt = (
        "/bucket/input/J2MM/speech/hdf5/mmasr_info/test/test_noalpha.info"
    )
    sampler_bucket_size = 10000
    checkpoint_path = "/bucket/input/J2MM/speech/hdf5/mmasr_pretrain_model/mmasr_baseline3_j2mm_1000id_image_aug_finetune_float-checkpoint-epoch-0069-2a753168.pth.tar"
else:
    # 本地训练的一些配置
    ...
    train_hdf5_audio_path = (
        "/horizon-bucket/J2MM/speech/hdf5/audio.hdf5"  # local
    )
    val_hdf5_audio_path = {
    ...
    ckpt_dir = (
        f"/mnt/mnt-data-2/baozhong.ma/Projects/HAT/tmp/models/{model_prefix}/"
    )
    epoch_log_freq = 1
    step_log_freq = 10
    ...
    test_info_txt = (
        "/horizon-bucket/J2MM/speech/hdf5/mmasr_info/test/test_noalpha.info"
    )
    sampler_bucket_size = 200
    checkpoint_path = "/horizon-bucket/J2MM/speech/hdf5/mmasr_pretrain_model/mmasr_baseline3_j2mm_1000id_image_aug_finetune_float-checkpoint-epoch-0069-2a753168.pth.tar"
...
# 数据增强的配置
augment_config = (
    "projects/halo/avspeech/data/noise/exp_mmasr_augment_filter.yaml"
)
with open(augment_config) as fr:
    aug_config = yaml.load(fr, Loader=yaml.FullLoader)

train_dataset = dict(
    type=CocktailDatasetV1,
    image_reader=dict(
...
FYZ_val_db_levels = ["FYZ 10db", "FYZ 0db", "FYZ m5db", "FYZ clean"]
QR_val_db_levels = ["QR 10db", "QR 0db", "QR m5db", "QR clean"]
...
# val_metric_updater["log_prefix"] = "VAL " + model_prefix
...
for db in test_db_level:
    db_val_hdf5_audio_path = os.path.join(
        test_hdf5_audio_path, f"QR_mmasr_test_mic1_{db}.hdf5"
    )
    test_dataset = dict(
        type=CocktailDatasetV1,
        image_reader=dict(
...
float_predictor = dict(
    ...
    model_convert_pipeline=dict(
        ...
        converters=[
            dict(
                type="LoadCheckpoint",
                checkpoint_path="/mnt/mnt-data-2/baozhong.ma/Projects/model/mmasr/float-checkpoint-epoch-0023-c7ed3af5.pth.tar",
            ),
        ],
    ),
