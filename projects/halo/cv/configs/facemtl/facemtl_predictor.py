...
# -------------------------- common --------------------------
from common import expand_type  # noqa
from common import (
    batch_transforms,
    device_ids,
    input_hw,
...
ckpt_dir = "/home/users/jiaqi.quan/model/face_mtl/"
...
test_data_loaders = []
# collect test_metric of facequality
...
# collect test_metricr of ldmk
ldmk_test_metric_list = []
for T in TASK_CONFIGS:
    if T.task_name == "face_ldmk":
        ldmk_test_metric_list = T.test_metrics
for ldmk_test_metric in ldmk_test_metric_list:
    test_metric_list.append(ldmk_test_metric)

# collect test_metric of face3d
face3d_test_metric_list = []
for T in TASK_CONFIGS:
    if T.task_name == "face3d_finetune":
        face3d_test_metric_list = T.test_metrics
for face3d_test_metric in face3d_test_metric_list:
    test_metric_list.append(face3d_test_metric)


# collect test_data_loader: facequality
...
# collect test_data_loader: ldmk
loader_key = tuple(["face_ldmk"])  # noqa
ldmk_test_loader_list = []
for T in TASK_CONFIGS:
    if T.task_name == "face_ldmk":
        ldmk_test_loader_list = T.test_data_loader_list
for ldmk_test_loader in ldmk_test_loader_list:
    test_data_loaders.append(
        dict(
            ...
            return_task=True,
            mode="validation",
            loaders={loader_key: ldmk_test_loader},
            __build_recursive=True,
        )
    )

# collect test_data_loader: face3d
loader_key = tuple(["face3d"])  # noqa
face3d_test_loader_list = []
for T in TASK_CONFIGS:
    if T.task_name == "face3d":
        face3d_test_loader_list = T.test_data_loader_list
for face3d_test_loader in face3d_test_loader_list:
    test_data_loaders.append(
        dict(
            type="MultitaskLoader",
            task_sampler=None,
            return_task=True,
            mode="validation",
            loaders={loader_key: face3d_test_loader},
            __build_recursive=True,
        )
    )
...
# collect test_metric_updater of facequality
...
# collect test_metric_updater of ldmk
ldmk_test_metric_updater_list = []
for T in TASK_CONFIGS:
    if T.task_name == "face_ldmk":
        ldmk_test_metric_updater_list = T.test_metric_updater_list
for ldmk_test_metric_updater in ldmk_test_metric_updater_list:
    test_metric_updaters.append(ldmk_test_metric_updater)

# collect test_metric_updater of face3d
face3d_test_metric_updater_list = []
for T in TASK_CONFIGS:
    if T.task_name == "face3d":
        face3d_test_metric_updater_list = T.test_metric_updater_list
for face3d_test_metric_updater in face3d_test_metric_updater_list:
    test_metric_updaters.append(face3d_test_metric_updater)
        ...
        converters=[
            dict(
                ...
                checkpoint_path=os.path.join(
                    ckpt_dir, "freeze_bn-checkpoint-epoch-last.pth.tar"
                ),
                allow_miss=False,
                ignore_extra=True,
