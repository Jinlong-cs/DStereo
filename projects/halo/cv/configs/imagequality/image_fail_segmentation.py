...
model_name = "halo_" + task_name + model_version
seed = None
...
ds_path_template = os.path.join(
    os.path.dirname(__file__),
    "image_fail_seg_{}.py",
)
ds_path = ds_path_template.format("dataset")
...
deploy_inputs = dict(img=torch.randn((1, 3, 320, 512)))
...
# -------------------------- step float --------------------------
# pretrain_checkpoint = "http://fm-tian-li.alitrain.hogpu.cc/plat_gpu/multitask-pretrain-2631-20211109-130832/output/models/vargnetv2_cls/float-checkpoint-best.pth.tar"
pretrain_checkpoint = "http://fm-qi-xiang.alitrain.hogpu.cc/plat_gpu/hobot-dag-1980054_image-fail-parsing-vargnet-float3w-fbn5k-qat5k-cls5-320x512-data-clean-20230314-110807/output/models/image_fail_parsing/qat-checkpoint-last-db741431.pth.tar"  # noqa
float_trainer = deepcopy(base_trainer)
...
# -------------------------- step freeze_bn_1 --------------------------
freeze_bn_1_trainer = deepcopy(base_trainer)
freeze_bn_1_trainer.update(
    dict(
        model_convert_pipeline=dict(
            type="ModelConvertPipeline",
            converters=[
...
# -------------------------- step freeze_bn_2 --------------------------
freeze_bn_2_trainer = deepcopy(base_trainer)
freeze_bn_2_trainer.update(
    dict(
        model_convert_pipeline=dict(
            type="ModelConvertPipeline",
            converters=[
...
# -------------------------- step qat --------------------------
qat_trainer = deepcopy(base_trainer)
qat_trainer.update(
    dict(
        model_convert_pipeline=dict(
            type="ModelConvertPipeline",
