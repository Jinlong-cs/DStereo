...
if local:
    ...
    job_id = ""
    # job_id = "finetune-all-0307-faceid-more-50-epoch-decay025-mix05-large-fc-0327"
    # root_dir = "/home/users/han.tang/workspace/model_result/fatigue"
    # job_id = os.environ.get("JOB_ID", "eye_status_baseline_reproduce")
    num_workers = 5
    # save_feat_dir = f"/home/users/han.tang/data/fatigue/eval-feat/{job_id}"
    save_feat_dir = (
        f"/home/users/jiaqi.quan/project/fatigue/eval-feat/{job_id}"
    )
    # save_feat_dir = '/home/users/jiaqi.quan/project/fatigue/eval-feat/Volkswagen/'
else:
    if save_bucket:
        root_url = "dmpv2://MultiMode/han.tang/workspace/HAT/model_result/fatigue/float_models/mixvargenet-all/"
        root_dir = bucket_client.url_to_local(root_url)
        job_id = os.environ.get("JOB_ID", "")
        job_id = f"finetune-all-1213-faceid-more-50-epoch-decay025-mix05-large-fc-{job_id}"
    else:
        root_dir = "/job_data/"
        job_id = "debug"
    num_workers = 12
...
pretrain_url = "dmpv2://MultiMode/han.tang/workspace/HAT/model_result/fatigue/float_models/mixvargenet-all/pretrain-faceid-more-50-epoch-decay025-mix05-large-fc"
batch_size_per_gpu = 512
device_ids = [0, 1]

ckpt_dir = os.path.join(root_dir, job_id)
# ckpt_dir = '/home/users/jiaqi.quan/project/fatigue/model/Volkswagen/'
...
if local:
    # data_root = "/home/users/han.tang/data/fatigue/train-data/"
    ...
    data_root = (
        "/horizon-bucket/MultiMode_3/jiaqi.quan/fatigue/test-data/20230331/"
    )
    # data_root = "/horizon-bucket/MultiMode_3/jiaqi.quan/fatigue/test-data/20230321/"
else:
...
rec_paths = [
    ...
    "eye_status_tanghan_20230327_train.rec",
    "eye_status_tanghan_20230331_train.rec",
    "eye_status_tanghan_20230413_train.rec",
    "eye_status_tanghan_20230421_train.rec",
]
...
def update_eval_metric(metrics, batch, model_outs):
    ...
    close_tp_fp_ind = pred_ind == close_cls_ind
    close_tp_tn_ind = target_ind == close_cls_ind
    ...
    for metric in metrics:
        if isinstance(metric.name, list):
            ...
            if metric.name.find("left_eye_acc") == 0:
                ...
                pred_cls = pred_cls.view(-1)[valid_inds]
                metric.update(label_cls, pred_cls)
            elif metric.name.find("eye_close_precision") == 0:
                metric.update(
                    torch.argmax(cls_target[close_tp_fp_ind], dim=1),
                    pred[
                        close_tp_fp_ind,
                    ],
                )
            elif metric.name.find("eye_close_recall") == 0:
                metric.update(
                    torch.argmax(cls_target[close_tp_tn_ind], dim=1),
                    pred[
                        close_tp_tn_ind,
                    ],
                )
            elif metric.name.find("eye_status_metrics") == 0:
...
#         if metric.name.find("left_eye_acc") == 0:
#             metric.update(
#                 torch.argmax(l_cls_target, dim=1),
#                 pred[
#                     :batch_size,
#                 ],
#             )
#         elif metric.name.find("right_eye_acc") == 0:
#             metric.update(
#                 torch.argmax(r_cls_target, dim=1),
#                 pred[
#                     batch_size:,
#                 ],
#             )
...
float_predictor = dict(
        ...
        converters=[
            dict(
                ...
                checkpoint_path=os.path.join(
                    ckpt_dir,
                    "float-checkpoint-step-11999.pth.tar"
                    # "float-checkpoint-step-9999.pth.tar"
                ),
                ignore_extra=True,
                allow_miss=True,
                verbose=True,
    ...
    metrics=[
        ...
        # dict(
        #     type="Accuracy",
        #     name="eye_close_precision",
        # ),
        # dict(
        #     type="Accuracy",
        #     name="eye_close_recall",
        # ),
    ],
    callbacks=[
        val_metric_updater,
    ...
    model_convert_pipeline=dict(
        ...
        converters=[
            ...
            dict(
                ...
                checkpoint_path=os.path.join(
                    ckpt_dir,
                    "qat-checkpoint-step-12999.pth.tar"
