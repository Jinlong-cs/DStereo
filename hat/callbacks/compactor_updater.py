...
class CompactorUpdater(CallbackMixin):
    ...
    Args:
        before_mask_iters: skip some iterations before mask
        mask_interval: the interval iterations between masks
        pruned_epsilon: the threshold to select filters
        modules: the pruned modules
    """

    def __init__(
        self,
        before_mask_iters: int,
        mask_interval: int,
        pruned_epsilon: float,
        modules: List,
    ):
