...
class MetricUpdater(CallbackMixin):
    ...
    Args:
        ...
        step_storage_freq: Storage every `step_storage_freq` steps. If < 1,
            disable step storage.
        epoch_storage_freq: Storage every `epoch_storage_freq` epochs. If < 1,
            disable epoch storage.
    ...
    def __init__(
        ...
        self.step_storage_freq = step_storage_freq
        self.epoch_storage_freq = epoch_storage_freq
        ...
        for dataset in kwargs['data_loader'].dataset.datasets:
            if isinstance(dataset, ConcatDataset):
                for sub_dataset in dataset.datasets:
                    ...
                    #     message += "[%0.1f:%0.1f]: %d\t" % (bin_edges[idx], bin_edges[idx + 1], val)
    ...
    def on_step_end(
        ...
        **kwargs,
        ...
        if self.step_log_freq > 0 and (step_id + 1) % self.step_log_freq == 0:
            ...
            if StorageHandler.producer_acked(
                storage, StorageHandler.metrics_keys_step
            ):  # noqa E501
                self._metric_storage(storage, metrics, "step")
    ...
    def on_epoch_end(
        ...
        **kwargs,
        ...
        if (
            self.epoch_storage_freq > 0
            and (epoch_id + 1) % self.epoch_storage_freq == 0
        ):
            name_prefix = "Epoch[%d]_" % epoch_id
            self._monitor_storage(metrics, storage, name_prefix)

        if (epoch_id + 1) % self.epoch_log_freq == 0:
            prefix = "Epoch[%d] %s: " % (epoch_id, self.log_prefix)
            if StorageHandler.producer_acked(
                storage, StorageHandler.metrics_keys_epoch
            ):  # noqa E501
                self._metric_storage(storage, metrics, "epoch")
            self._log(metrics, prefix)
    ...
    def _metric_storage(
        ...
        if metric_interval == "step":
            storage_key = StorageHandler.metrics_keys_step
        else:
            storage_key = StorageHandler.metrics_keys_epoch
        for metric in metrics:
            name, value = metric.get()
            for k, v in zip(_as_list(name), _as_list(value)):
                if isinstance(v, (int, float, str, EvalResult)):
    ...
    def _monitor_storage(
        ...
        for m in metrics:
            name, value = m.get()
            for k, v in zip(_as_list(name), _as_list(value)):
                storage.put(self.storage_key, (name_prefix + k, v))
