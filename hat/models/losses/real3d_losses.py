...
from hat.models.base_modules.activation import DynamicWeight
...
def sigmoid_and_clip(x, min_value=1e-4, max_value=1 - 1e-4):
    y = torch.clamp(x.sigmoid(), min=min_value, max=max_value)
    return y


def softmax_and_clip(x, dim=1, min_value=1e-4, max_value=1 - 1e-4):
    y = torch.clamp(x.softmax(dim=dim), min=min_value, max=max_value)
    return y
...
def hm_focal_loss(
    ...
    beta: int = 1,
    alpha: int = 4,
    pos_cls_weight: Optional[torch.Tensor] = None,
    neg_cls_weight: Optional[torch.Tensor] = None,
):
    """
    Heatmap focal loss.
    ...
    Args:
        ...
        gt (torch.Tensor): learning label of the prediction.
        ignore_mask (torch.Tensor): Weather to ignore.
        gamma (int): default to 2.
        beta (int): default to 1.
        alpha (int): default to 4.
        pos_cls_weight(torch.Tensor, Optional):
            weight of positive samples. Default to None.
        neg_cls_weight(torch.Tensor, Optional):
            weight of negative samples. Default to None.
    Returns:
        return losses.
    """
    ...
    if pos_cls_weight is not None:
        pos_loss = pos_loss * pos_cls_weight
    if neg_cls_weight is not None:
        neg_loss = neg_loss * neg_cls_weight
...
def hm_l1_wing_loss(
    ...
    """
    Heatmap L1 wing loss, refer to https://arxiv.org/pdf/1711.06753.pdf.

    Args:
        output: model prediction.
        target: learning label of the prediction.
        weight_mask: weight of hm samples.
        ignore_mask: whether to ignore.
        avg_factor: normalized factor of loss.
        omega: default to 1.0.
        sigma: default to 0.3.
            Should not be too small for large prediction range.
    ...
    interval_mask = (torch.abs(output - target) < omega).float()
    loss1 = omega * torch.log(1 + torch.abs(output - target) / sigma)
    loss2 = torch.abs(output - target) - (
        omega - omega * torch.log(1.0 + torch.div(omega, sigma))
    )
    l1_wing_loss = (
        (loss1 * interval_mask + loss2 * (1.0 - interval_mask)) * weight_mask
    ).sum()

    loss = l1_wing_loss / (EPS + avg_factor)
    return loss
