...
class CenterPrior(nn.Module):
    ...
    def forward(
        self, anchor_points_list, gt_bboxes, labels, inside_gt_bbox_mask
    ):
        ...
        for slvl_points, stride in zip(anchor_points_list, self.strides):
            ...
            distance = (
                (single_level_points - gt_center) / float(stride)
                - instance_center
            ) ** 2
...
class AutoAssignTarget(object):
    ...
    def _get_ignore_mask(gt_labels_list, max_avg_num_gt):
        ...
        assert (
            max_avg_num_gt is None or max_avg_num_gt > 0
        ), "max_avg_num_gt \
        ...
        if max_avg_num_gt is not None:
            valid_num_gt_list = [
                (gt_labels >= 0).sum() for gt_labels in gt_labels_list
            ]
            ...
            if total_pruning_num > 0:
                ...
                deficient_num = sum(
                    [
                        -offset_num
                        for offset_num in offset_num_list
                        if offset_num < 0
                    ]
                )
                redundant_num = sum(
    ...
    def get_targets(
        self, points, gt_bboxes_list, gt_ignore_mask_list, valid_classes_list
    ):
    ...
    def _get_target_single(
        self, gt_bboxes, gt_ignore_mask, valid_class, points
    ):
        """Compute regression targets and each point inside or outside gt_bbox \
        for a single image.
        ...
        if gt_ignore_mask is None:
            inside_gt_bbox_mask = gt_bboxes.new_zeros(
                (num_points, 0), dtype=torch.bool
            )  # noqa
    ...
    def __call__(self, label, pred, *args):
        ...
        gt_ignore_mask_list = self._get_ignore_mask(
            gt_labels_list, self.max_avg_num_gt
        )
        ...
        tmp_gt_labels_list = []
        tmp_gt_bboxes_list = []
        for gt_label, gt_bboxe, gt_ignore_mask in zip(
            gt_labels_list, gt_bboxes_list, gt_ignore_mask_list
        ):
            if gt_ignore_mask is None:
        ...
        for bbox_pred, gt_bboxe, inside_gt_bbox_mask in zip(
            ...
            expand_bbox_pred = (
                bbox_pred[:, None, :]
                .expand(num_points, temp_num_gt, 4)
                .reshape(-1, 4)
            )
            decoded_bbox_preds = distance2bbox(
