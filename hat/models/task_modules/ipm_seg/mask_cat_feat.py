...
class MaskcatFeatHead(nn.Module):
    ...
    Args:
        ...
        level_indices: A list of index of neck features input to head.
            If given, start_level and end_level will be ignored.
            Default is None.
        ...
        has_project_layer: Whether add projection layer in head.
            Default: False
        auxi_use_bifpn: Whether use bifpn features as auxi-layers input.
            Default: True
        ...
        upsample_output_scale: If upsample output to input scale.
    ...
    def __init__(
        ...
        start_level: int = 0,
        end_level: int = 2,
        level_indices: Optional[Iterable] = None,
        group_base: int = 8,
        ...
        if level_indices is None:
            level_indices = list(range(start_level, end_level + 1))
        ...
        self.auxi_use_bifpn = auxi_use_bifpn
        ...
        if upsample_to_input:
            self.last_conv3x3 = base_conv_module(
                self.out_channels,
                self.out_channels,
                kernel_size=3,
                stride=1,
        ...
        if self.upsample_output_scale:
            self.upsample_output = []
            for i in range(len(self.out_strides)):
                one_upsample = Interpolate(
                    scale_factor=self.out_strides[i],
                    mode="bilinear",
                    align_corners=None,
                    recompute_scale_factor=True,
                )
                self.upsample_output.append(one_upsample)
        self._init_weights()
    ...
    def _init_fpn_layers(self):
        ...
        strides = [self.in_strides[i] for i in self.level_indices]
        scale_factors = [
    ...
    def forward(self, inputs):
        ...
        assert len(inputs) >= len(self.level_indices)

        inputs = [inputs[i] for i in self.level_indices]
        # feature aggregation
        project_feats = []
        if self.has_project_layer:
        ...
        if self.aggregation_method == "concat":
            feature_add_all_level = self.cat_layer.cat(
        ...
        # seg_cls
        preds = []
        if not self.upsample_to_input:
            pred = self.conv_cls_layers[0](seg_feats[0])
            pred = self.upsample(pred)
        else:
            upsample_feat = self.upsample(seg_feats[0])
            upsample_feat = self.last_conv3x3(upsample_feat)
            pred = self.conv_cls_layers[0](upsample_feat)

        if self.upsample_output_scale:
            pred = self.upsample_output[0](pred)
        if self.argmax_output:
            pred = horizon.argmax(pred, dim=1, keepdim=True)
        ...
        if self.use_auxi_loss:
            ...
            if self.upsample_output_scale:
                for i in range(1, len(inputs)):
                    preds[i] = self.upsample_output[i](preds[i])
        return tuple(preds)
