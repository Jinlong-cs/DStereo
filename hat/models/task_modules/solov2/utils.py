...
def dynamic_upsample(
    prediction: torch.Tensor, upsample_factor: torch.Tensor, stride: int
):
    """Dynamic upsampling."""

    N, C, H, W = prediction.shape
    upsample_factor = upsample_factor.view(1, 1, 9, stride, stride, H, W)
    upsample_factor = torch.softmax(upsample_factor, dim=2)

    up_prediction = F.unfold(prediction, [3, 3], padding=1)
    up_prediction = up_prediction.view(N, C, 9, 1, 1, H, W)

    up_prediction = torch.sum(upsample_factor * up_prediction, dim=2)
    up_prediction = up_prediction.permute(0, 1, 4, 2, 5, 3)
    return up_prediction.reshape(N, C, stride * H, stride * W)
...
def mask_matrix_nms(
    ...
    iou_matrix = (
        ...
        / (
            expanded_mask_area
            + expanded_mask_area.transpose(1, 0)
            - inter_matrix
        )
    ).triu(diagonal=1)
    ...
    compensate_iou = compensate_iou.expand(num_masks, num_masks).transpose(
        1, 0
    )
...
def center_of_mass(gt_mask):
    center_h, center_w = torch.nonzero(gt_mask).float().mean(0)
