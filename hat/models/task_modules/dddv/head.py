...
class FusionResidualFlowBlock(nn.Module):
    ...
    Args:
        channels: Feature map channle.
        upsampling: Wheather upsample resflow.
            Only setting False in stride32.
        dequant_out: Wheather dequanti output.
...
class ResidualFlowPoseHead(nn.Module):
    ...
    Args:
        ...
        has_resflow: output res_flow or not.
...
class OutputBlock(nn.Module):
    ...
    Args:
        in_channels: in_channels for feature map.
        out_channels: out_channels for feature map.
        ...
        dequant_out: whether dequant the output.
        dropout_ratio: dropout  ratio, should be in 0 ~ 1.
        bn_kwargs: kwargs of bn layer.
        use_bias: whether to use bias.
        factor: factor of Group Separable Conv.
        batchnorm_output: whether to add batch norm to output
            last conv.
    ...
    def __init__(
        ...
        self.batchnorm_output = batchnorm_output
        if batchnorm_output:
            self.result_block = ConvModule2d(
                ...
                padding=0,
                bias=False,
                norm_layer=nn.BatchNorm2d(out_nums, **bn_kwargs),
            )
        else:
    ...
    def fuse_model(self):
        ...
        if self.batchnorm_output:
            self.result_block.fuse_model()
...
class PixelHead(nn.Module):
    ...
    Args:
        ...
        forward_frame_idxs: when input multi frame features,
    ...
    def forward(self, data: Union[Sequence, Mapping]):
        res = OrderedDict()
        _data = data[self.feature_name] if isinstance(data, Mapping) else data
        for frame_idx, features in enumerate(_data):
...
class DepthPoseResflowHead(nn.Module):
    ...
    Args:
        depth_head: config of depth head to build.
        pose_resflow_head: config of pose_resflow head to build.
        feature_name: Name of features from backbone(or neck)
            in input dict.
