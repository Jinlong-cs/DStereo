...
class RCNNDecoder(nn.Module):
    ...
    Args:
        bbox_decoder: Bbox decoder.
        cls_act_type: Classification output activation type,
            should be one of ['sigmoid', 'identity', 'softmax'].
        legacy_bbox: Whether is legacy bbox.
        im_hw: Image (height, width).
    ...
    def __init__(
        ...
        min_filter_hw: Optional[bool] = None,
    ...
    def forward(
        ...
        if self.clip_by_im_hw:
            batch_boxes = bbox_clamp(batch_boxes, self.im_hw, self.legacy_bbox)
        if self.min_filter_hw is not None:
            mask = bbox_filter_by_hw(
                batch_boxes, self.min_filter_hw, self.legacy_bbox
            )
            batch_boxes = batch_boxes * mask
            batch_cls_pred = batch_cls_pred * mask
