...
def affine_transform_nd(pt, t):
    return (t[:2, :2] * pt[..., None]).sum(dim=-1) + t[:2, 2]
...
class ROI3DDecoder(nn.Module):
    ...
    def forward(
        ...
        if self.undistort_depth_uv:
            ...
            pre_depths = head_out["depth_pred"]
        ...
        for i, (
            ...
            affine_center_2d = get_undistort_points(
            ...
            if self.undistort_depth_uv:
                ...
                max_depth = (
                    max_depth * calib[i, 0, 0] / self.focal_length_default
                )
