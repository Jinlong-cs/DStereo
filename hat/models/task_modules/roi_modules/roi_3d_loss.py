# cloneright (c) Horizon Robotics. All rights reserved.
import copy
...
from hat.models.task_modules.real3d.camera3d_loss import (
...
def generate_depth_weight(iou):

    iou = iou.view(-1, 1, 1, 1)
    iou = iou.expand(iou.shape[0], 1, 8, 8)

    weight = torch.exp(iou)
    return weight


def exp_iou_label(iou_label):
    iou_label = torch.sigmoid(torch.exp(iou_label) - 1)
    return iou_label
...
class RCNNSparse3DLoss(nn.Module):
    ...
    Args:
        ...
        use_rotation_iou: whether to use rotation iou.
        use_exp_iou_label: whether to use iou exp label.
        adjust_depth_loss_by_iou: whether adjust depth loss by iou.
    """
    ...
    def __init__(
        ...
        use_rotation_iou: bool = False,
        use_exp_iou_label: bool = False,
        adjust_depth_loss_by_iou: bool = False,
    ):
        ...
        self.use_rotation_iou = use_rotation_iou
        self.use_exp_iou_label = use_exp_iou_label
        self.adjust_depth_loss_by_iou = adjust_depth_loss_by_iou
    ...
    def forward(
        ...
        if self.loss_custom_weight is not None:
            loss_custom_weight = targets["loss_custom_weight"]
            loss_custom_weight_sparse = loss_custom_weight
            if isinstance(loss_custom_weight, list):
                loss_custom_weight = loss_custom_weight[0]
                loss_custom_weight_sparse = copy.deepcopy(
                    self.loss_custom_weight
                )
                for key in loss_custom_weight:
                    batch_size = loss_custom_weight[key].shape[0]
                    for index in range(batch_size):
                        loss_custom_weight_sparse[key][index][
                            ...
                        ] = loss_custom_weight[key][
                            index
                        ]  # shape: (size, 128)
                for key in loss_custom_weight:
                    loss_custom_weight_sparse[key] = loss_custom_weight_sparse[
                        key
                    ].to(loss_custom_weight[key].device)
                    if key == "sparse_dim" or key == "sparse_iou":
                        loss_custom_weight_sparse[
                            key
                        ] = loss_custom_weight_sparse[key].view(
                            -1, 1
                        )  # shape: (size*128, 1)
                    elif key == "sparse_rot":
                        continue  # shape: (size, 128)
                    else:
                        loss_custom_weight_sparse[
                            key
                        ] = loss_custom_weight_sparse[key].view(
                            -1, 1, 1, 1
                        )  # shape: (size*128, 1, 1, 1)
        ...
        if self.loss_custom_weight:
            cls_weight *= loss_custom_weight_sparse[
                "sparse_center_2d"
            ]  # [128, 1, 8, 8] * [size, 128]
        ...
        if self.loss_custom_weight:
            offset_2d_weight *= loss_custom_weight_sparse["sparse_offset_2d"]
        ...
        if self.loss_custom_weight:
            offset_3d_weight *= loss_custom_weight_sparse["sparse_offset_3d"]
        ...
        if self.loss_custom_weight:
            pos_match *= loss_custom_weight_sparse["sparse_dim"]
        ...
        if self.loss_custom_weight:
            batch_mask *= loss_custom_weight_sparse["sparse_rot"]
        ...
        if self.adjust_depth_loss_by_iou:
            depth_weight_by_iou = generate_depth_weight(iou_label)
            depth_weight_by_iou = depth_weight_by_iou * depth_weight
        else:
            depth_weight_by_iou = depth_weight
        ...
        if self.undistort_depth_uv:
            ...
            depth_u_loss = self.depth_loss(
                ...
                weight=depth_weight_by_iou
                * loss_custom_weight_sparse["sparse_depth_u"]
                if self.loss_custom_weight
                else depth_weight_by_iou,
            ...
            depth_v_loss = self.depth_loss(
                ...
                weight=depth_weight_by_iou
                * loss_custom_weight_sparse["sparse_depth_v"]
                if self.loss_custom_weight
                else depth_weight_by_iou,
                avg_factor=(depth_weight.sum(dim=[1, 2, 3]) > 0).sum() + 1e-6,
            )
            ...
            depth_loss = self.depth_loss(
                ...
                target=depth_label,
                weight=depth_weight_by_iou
                * loss_custom_weight_sparse["sparse_depth"]
                if self.loss_custom_weight
                else depth_weight_by_iou,
                avg_factor=(depth_weight.sum(dim=[1, 2, 3]) > 0).sum() + 1e-6,
            )
        ...
        iou_label = (
            iou_label
            if not self.use_exp_iou_label
            else exp_iou_label(iou_label)
        )
        ...
        if self.loss_custom_weight:
            iou_loss *= loss_custom_weight_sparse["sparse_iou"]
...
def corner_loss_v2(
    ...
    if loss_custom_weight is not None:
        rot_loss *= loss_custom_weight
...
def cal_bev_iou(
    ...
    if use_rotation_iou:
        gt_bev_arr = gt_bev.cpu().numpy()
        pred_bev_arr = pred_bev.cpu().numpy()
        bs, prop_num, _, _ = gt_bev_arr.shape
        iou = torch.zeros((bs, prop_num, 1), dtype=torch.float32).to(
            gt_bev.device
        )
        for gi in range(bs):
            for pi in range(prop_num):
                iou[gi, pi, 0] = cal_iou_by_polygon(
                    gt_bev_arr[gi, pi, :, :], pred_bev_arr[gi, pi, :, :]
                )
    else:
        ...
        w = torch.minimum(pre_max_x, gt_max_x) - torch.maximum(
            pre_min_x, gt_min_x
        )
        w = torch.where(w < 0, torch.zeros_like(w), w)
        h = torch.minimum(pre_max_y, gt_max_y) - torch.maximum(
            pre_min_y, gt_min_y
        )
