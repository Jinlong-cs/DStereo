...
from hat.core.affine import get_affine_transform
...
class Real3DDecoder(torch.nn.Module):
    ...
    def forward(
        ...
        if "target" in label and "M" in label["target"].keys():
            inv_Ms = [
                np.linalg.inv(
                    np.concatenate(
                        [
                            label["target"]["M"][ind].cpu().numpy(),
                            np.array([[0, 0, 1]]),
                        ],
                        axis=0,
                    )
                )[:2, :]
                for ind in range(len(image_original_size))
            ]
        else:
            inv_Ms = [
            ...
            for i in range(len(center)):
                ...
                if self.fisheye:
                    ct_undistort = cv2.fisheye.undistortPoints(
                        ct_np, camera, dist_coeffs_i, None, camera
                    ).squeeze()
                else:
                    ct_undistort = cv2.undistortPoints(
                        ct_np, camera, dist_coeffs_i, None, camera
                    ).squeeze()
