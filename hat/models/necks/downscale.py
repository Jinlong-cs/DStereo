...
__all__ = ["ConvDownscaleNeck"]


@OBJECT_REGISTRY.register
class ConvDownscaleNeck(nn.Module):
    """Neck for downscale with fix ration using a 3x3 conv.

    There will be a 3x3 conv with stride 2 following each output feature map.
    ...
    Args:
        ...
        out_channels: channels of each output feature maps
    ...
    def __init__(
        ...
        super(ConvDownscaleNeck, self).__init__()
        ...
        for s in out_strides:
            assert s // 2 in in_strides
            out_indices.append(in_strides.index(s // 2))
        ...
        for idx, s in zip(out_indices, out_strides):
            self.downscale[f"stride_{s}"] = ConvModule2d(
    ...
    def forward(self, features: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        for idx, s in zip(self.out_indices, self.out_strides):
            outputs.append(self.downscale[f"stride_{s}"](features[idx]))

        return outputs

    def fuse_model(self):
        for module in self.downscale.values():
            module.fuse_model()
