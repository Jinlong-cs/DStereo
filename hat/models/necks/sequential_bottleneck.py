...
class SequentialBottleNeck(nn.Module):
    def __init__(
        ...
        Args:
            ...
            use_secnet: whether to build second neck block.
        ...
        if use_scnet:
            ...
            make_layer = self._make_second_layer
        else:
            make_layer = self._make_layer

        self.cat = horizon.nn.quantized.FloatFunctional()
    ...
    def _make_second_layer(
        ...
        **kwargs
        ...
        """Create original second layer structure."""
        block = nn.Sequential(
            nn.Identity(),
            ConvModule2d(
                in_channels=inplanes,
                ...
                padding=1,
                norm_layer=nn.BatchNorm2d(planes, **self._bn_kwargs),
                act_layer=copy.deepcopy(self.act_layer),
            ),
        )
    ...
    def forward(self, x: torch.Tensor, quant=False):
