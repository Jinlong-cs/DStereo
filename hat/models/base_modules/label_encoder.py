...
from hat.core.box_utils import box_corner_to_center, zoom_boxes
...
@OBJECT_REGISTRY.register
class MatchLabelTrackEncoder(nn.Module):
    """Encode gt and matching results to track labels.

    Args:
        track_use_pos_only: Whether to use positive labels only
            during encoding.
    ...
    def __init__(
        self,
        track_use_pos_only: bool = True,
        track_on_hard: bool = False,
        track_label_encoder: Optional[nn.Module] = None,
    ):
        super().__init__()
        self.track_use_pos_only = track_use_pos_only
        self.track_on_hard = track_on_hard
        self.track_label_encoder = track_label_encoder
    ...
    def forward(
        ...
        Args:
            boxes (torch.Tensor): (B, N, 4), batched predicted boxes
            gt_boxes (torch.Tensor): (B, M, 6+), batched ground
                truth boxes (x1, y1, x2, y2, cls, track_id, ...),
                might be padded.
            match_pos_flag (torch.Tensor): (B, N) matched result
                of each predicted box
            match_gt_id (torch.Tensor): (B, M) matched gt box index
        ...
        class_label = matched_gt_boxes[..., 4]
        gt_id = matched_gt_boxes[..., 5]

        # set track id of negative matchings to 0
        # track_id > 0 : positive
        # track_id = 0 : negative
        # track_id < 0 : ignore
        class_label[match_pos_flag == 0] = 0
        gt_id[match_pos_flag == 0] = 0

        out_dict = OrderedDict()

        # set track id of ignored instances to be -abs(label)
        ig_index = match_pos_flag < 0
        if not self.track_on_hard:
            ig_index = ig_index | (class_label < 0)
        gt_id[ig_index] = -gt_id[ig_index].abs()

        if self.track_use_pos_only:
            gt_id[match_pos_flag == 0] = -1

        if self.track_label_encoder is not None:
            gt_id = self.track_label_encoder(gt_id, class_label)

        # optionally set track id to ignore
        if ig_flag is not None:
            # gt_id[(ig_flag == 1) * (gt_id == 0)] = -1
            gt_id[ig_flag == 1] = -1

        gt_id = gt_id.flatten(start_dim=1, end_dim=-2)
        # gt_id_mask = gt_id >= 0

        out_dict.update(
            track_id=gt_id,
            # track_id_mask=gt_id_mask,
        )
        return out_dict


@OBJECT_REGISTRY.register
class ClassWiseTrackIdEncoder(nn.Module):
    ...
    def forward(self, track_id: torch.Tensor, cls_label: torch.Tensor) -> torch.Tensor:
        ...
        all_pos_label = cls_label.detach().clone()
        all_pos_label[label_ignore_mask] *= -1
        track_id = track_id.type(torch.long)

        encoded_track_id = -1 * torch.ones(
            (*(track_id.shape), self._num_classes),
            dtype=torch.long,
            device=track_id.device,
        )
        for cls in range(self._num_classes):
            cls_idx = all_pos_label == cls
            encoded_track_id[..., cls][cls_idx] = track_id[cls_idx]

        if self._exclude_background:
            encoded_track_id = encoded_track_id[..., 1:]

        return encoded_track_id


@OBJECT_REGISTRY.register
class MultiClassMatchLabelSepEncoder(nn.Module):
    """Encode gt and matching results to separate bbox and class labels.

    Args:
        bbox_encoder: BBox label encoder
        class_encoder: Class label encoder
        bg_in_label: Whether the background in label index 0.
            Default to True.
    """
    ...
    def __init__(
        ...
        bbox_encoder: Optional[nn.Module] = None,
        class_encoder: Optional[nn.Module] = None,
        bg_in_label: bool = True,
    ):
        super().__init__()
        assert not (bbox_encoder is None and class_encoder is None)
        self.bbox_encoder = bbox_encoder
        self.class_encoder = class_encoder
        self.bg_in_label = bg_in_label
    ...
    def forward(
        ...
        if self.class_encoder is not None:
            ...
            # optionally set cls_label to ignore
            # only ignore negative
            if ig_flag is not None:
                cls_label[(ig_flag == 1) * (cls_label == 0)] = -1
                if self.bg_in_label:
                    # in ig_flag, when fg rois matched to the ignore region,
                    # must set to the rois relation bg cls label to -1.
                    # bg is in the cls index 0.
                    bg_ig_flag = ig_flag.sum(dim=-1) > 0
                    cls_label[:, :, 0][bg_ig_flag] = -1

            cls_label = cls_label.flatten(start_dim=1, end_dim=-2)
            cls_label_weight = (cls_label >= 0) * 1.0
            if self.bg_in_label:
                weight_norm_val = (cls_label_weight[:, :, 1:] >= 0).sum()
            else:
                weight_norm_val = cls_label_weight.sum()
            if weight_norm_val > 0:
                cls_label_weight[cls_label >= 0] = 1.0 / weight_norm_val

            out_dict.update(
                cls_label=cls_label,
                cls_label_mask=cls_label_weight,
            )

        if self.bbox_encoder:
            # (batch, num_rois, 4) --> (batch, num_rois, num_cls, 4) --> (batch, num_rois, num_cls*4)  # noqa
            num_cls = cls_label.shape[-1]
            reg_label = (
                self.bbox_encoder(boxes, matched_gt_boxes)
                .unsqueeze(-2)
                .repeat(1, 1, num_cls, 1)
                .flatten(start_dim=2)
            )

            reg_label_weight = cls_label_weight * (cls_label > 0)
            if self.bg_in_label:
            ...
            out_dict.update(
                reg_label=reg_label,
                reg_label_mask=reg_label_weight,
            )
        return out_dict


@OBJECT_REGISTRY.register
class PersonPositionLabelFromMatch(nn.Module):
    ...
    def __init__(
        self,
        dms_position_classes_weight: int,
        oms_position_classes_weight: int,
    ):
        super().__init__()
        self.dms_position_classes_weight = dms_position_classes_weight
        self.oms_position_classes_weight = oms_position_classes_weight
    ...
    def forward(
        ...
        **kwargs
        ...
        Args:
            boxes: (B, N, 4), batched predicted boxes
            gt_boxes: (B, M, 7), batched ground truth boxes,
                might be padded if gt_box is different in each sample.
                (B, M, 0:3): gt boxes coordinates
                (B, M, 4): gt boxes class label.
                (B, M, 5): gt boxes label type. 0: DMS; 1: OMS.
                (B, M, 6): gt boxes position class label.
            match_pos_flag: (B, N), matched result of each predicted
                box, Entries with value 1 represents positive
                in matching, 0 for neg and -1 for ignore.
            match_gt_id: (B, N), matched gt box index
        ...
        cls_label_weight_dms = boxes.new_zeros(cls_label_dms.shape)
        cls_label_weight_oms = boxes.new_zeros(cls_label_oms.shape)
        ...
        for i in range(batch_size_per_ctx):
            ...
            pos_gt_roi_boxes = matched_gt_boxes[i]
            # 0 represents DMS label, 1 represent OMS label
            if 0 in pos_gt_roi_boxes[:, 5]:
                gt_cls = pos_gt_roi_boxes[:, 6]
                cls_label_dms[i, : len(gt_cls)] = gt_cls
                ...
                if num_bfg > 0:
                    if self.dms_position_classes_weight is None:
                        ...
                        gt_weight = torch.tensor(
                            self.dms_position_classes_weight,
                            device=num_bfg.device,
                        )[gt_positive_cls_int]
                        a = gt_weight / num_bfg
                        cls_label_weight_dms[i][cls_label_dms[i] >= 0] = a
                        cls_label_weight_dms[i][cls_label_dms[i] >= 0] = (
                            gt_weight / num_bfg
                        )
            if 1 in pos_gt_roi_boxes[:, 5]:
                gt_cls = pos_gt_roi_boxes[:, 6]
                cls_label_oms[i, : len(gt_cls)] = gt_cls
                ...
                if num_bfg > 0:
                    if self.dms_position_classes_weight is None:
                        ...
                        cls_label_weight_oms[i][cls_label_oms[i] >= 0] = (
                            gt_weight / num_bfg
                        )

        return OrderedDict(
            dms_cls_label=cls_label_dms,
            dms_cls_label_weight=cls_label_weight_dms,
            oms_cls_label=cls_label_oms,
            oms_cls_label_weight=cls_label_weight_oms,
        )
