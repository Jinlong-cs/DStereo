...
BLOCK_CONFIG = {
    ...
    "repmixvarge_k3k3_f2": {
        "factor": 2,
        "conv1_kernel_size": {3: 1, 1: 1},
        "conv2_kernel_size": {3: 1, 1: 1},
        "use_reparam": True,
    },
    "repmixvarge_k3k3_f2_gb16": {
        "factor": 2,
        "conv1_kernel_size": {3: 1, 1: 1},
        "conv2_kernel_size": {3: 1, 1: 1},
        "use_reparam": True,
    ...
    "mixvarge_f1": {"conv2_kernel_size": 1, "padding": 0, "factor": 1},
...
class BasicMixVarGEBlock(nn.Module):
    ...
    def __init__(
        ...
        if use_reparam:
            block_type = OnlineReParamBlock
            conv1_pad = None  # not use
        else:
            block_type = ConvModule2d
            conv1_pad = (conv1_kernel_size - 1) // 2
    ...
    def fuse_model(self):
        ...
        if self.use_reparam:
            self.conv[1].create_merge_conv()
            fuse_list = ["conv.1.merge_conv", "conv.1.norm_layer"]
        else:
