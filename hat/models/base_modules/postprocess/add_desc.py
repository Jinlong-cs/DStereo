...
from hat.utils.apply_func import _as_list, flatten, is_list_of_type, regroup
...
class AddDesc(PostProcessorBase):
    ...
    `horizon_plugin_pytorch.functional.get_output_annotation(script_model)`.
    ...
    def forward(
        ...
        *args,
        ...
        for x, desc in zip(flat_tensors, self.per_tensor_desc):
            horizon.set_annotation(x, desc)

        pred, tensor_idx = regroup(flat_tensors, fmt)
        assert len(flat_tensors) == tensor_idx, "regroup fail"
        return pred
