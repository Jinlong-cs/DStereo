...
@OBJECT_REGISTRY.register
class ViewFusionHbirInfer(BaseHbirModule):
    """
    ...
    def __init__(
        ...
        model_path: str,
        bev_decoder_infers: List[nn.Module],
    ):
        super(ViewFusionHbirInfer, self).__init__(model_path=model_path)
    ...
    def forward(self, data):
        ...
        hbir_outputs = super().forward(inputs)
...
@OBJECT_REGISTRY.register
class ViewFusion4DHbirInfer(ViewFusionHbirInfer):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(ViewFusion4DHbirInfer, self).__init__(**kwargs)
    ...
    def process_input(self, data):
        ...
        prev_points = (
            self.deploy_model.temporal_fusion.export_reference_points(data)
        )
        self.prev_points.data = torch.cat(
            [prev_points["prev_points"], self.prev_points[0:1]]
        )

        return (image, ref_p, self.prev_feats, self.prev_points)
