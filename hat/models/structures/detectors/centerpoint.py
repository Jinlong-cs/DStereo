...
__all__ = ["CenterPointDetector", "CenterPointDetectorHbirInfer"]
...
@OBJECT_REGISTRY.register
class CenterPointDetectorHbirInfer(BaseHbirModule):
    """
    ...
    def forward(self, example):  # noqa: C408
        ...
        hbir_outputs = super().forward(
            (data["features"], data["coors"].to(torch.int32))
        )
        results_convert = []
        start = 0
