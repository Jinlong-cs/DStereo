...
from hatbc.workflow.proxy import Variable, WorkflowVariable, get_traced_graph
from hatbc.workflow.symbol import Symbol
from hatbc.workflow.trace import GraphTracer, is_workflow_traceable
...
class GraphModel(nn.Module):
    ...
    def _build_graph(self, clone: bool = False):  # noqa: D205,D400
        ...
        traceable_classes = GraphTracer._TRACEABLE_CLASSES
        GraphTracer.register_class_traceable_under_scope(torch.nn.Module)
        ...
        # restore traceable classes
        GraphTracer._TRACEABLE_CLASSES = traceable_classes
    ...
    def forward(
        ...
        assert len(unexpected_keys) == 0, "%s not in input names: %s" % (
            unexpected_keys,
            self.inputs.keys(),
        )
