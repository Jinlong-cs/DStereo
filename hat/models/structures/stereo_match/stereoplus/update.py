...
class ConvGRU(nn.Module):
    ...
    def forward(self, h, c, x):
        ...
        # q = self.relu(self.convq(torch.cat([r * h, x], dim=1)) + c[2])
...
class BasicMotionEncoder(nn.Module):
    def __init__(self, cor_planes):
        ...
        # self.convc1 = nn.Conv2d(48, cor_planes, 1, padding=0)
        self.convc1 = nn.Conv2d(24, cor_planes, 1, padding=0)
        self.convc2 = nn.Conv2d(cor_planes, cor_planes, 3, padding=1)
        self.convd1 = nn.Conv2d(1, cor_planes, 7, padding=3)
        self.convd2 = nn.Conv2d(cor_planes, cor_planes, 3, padding=1)
        self.conv = nn.Conv2d(cor_planes + cor_planes, cor_planes - 1, 3, padding=1)
...
class BasicUpdateBlock(nn.Module):
    def __init__(self, hidden_dim=96):
        super().__init__()
        self.encoder = BasicMotionEncoder(hidden_dim)
        encoder_output_dim = hidden_dim

        self.gru = ConvGRU(hidden_dim, hidden_dim)
        self.disp_head = DispHead(hidden_dim, hidden_dim=48, output_dim=1)
