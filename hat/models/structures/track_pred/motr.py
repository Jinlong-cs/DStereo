...
@OBJECT_REGISTRY.register
class MotrHbirInfer(BaseHbirModule, Motr):
    ...
    def __init__(
        ...
        self.qim_model = load(qim_model_path)
        self.qim_input_flattener = get_hbir_input_flattener(self.qim_model)
        self.qim_output_unflattener = get_hbir_output_unflattener(
            self.qim_model
        )
    ...
    def forward(self, data: Dict):
        ...
        for i in range(self.batch_size):
            ...
            for frame_id in range(self.max_frame_num):
                ...
                if seq_name is None:
                    break
                flat_input = self.input_flattener(
                    (seq_data["img"][frame_id], track_pos, mask_query, ref_pts)
                )
                flat_input = [x.cpu().numpy() for x in flat_input]
                hbir_output = self.model.functions[0](*flat_input)
    ...
    def _frame_post_process(
        ...
        if pad_len:
            ...
            flat_input = self.qim_input_flattener(
                (pd_query_pos, pd_output_embedding, pd_mask_query)
            )
            flat_input = [x.cpu().numpy() for x in flat_input]
            embed_query_pos = self.qim_model.functions[0](*flat_input)
            embed_query_pos = self.qim_output_unflattener(embed_query_pos)
            embed_query_pos = self.output_to_tensor(embed_query_pos)
            padding_track_instance = self._pocess_track_embed_output(
                padding_track_instance,
                pad_len,
                embed_query_pos,
