...
def get_closest_points(loc, yaw_rad, length):
    ...
    closest_points = loc + addition
    return closest_points


def collect_data(batch, output, gt_cids, det_cids):
    ...
    for i in range(batch_size):
        for j in range(num_objs):
            pred = {
                key: val[i][j].cpu().numpy() for key, val in results.items()
            }
            pred["image_id"] = image_id[i]
            cid = pred["category_id"].tolist()
            if cid in _det_group_by_cid:
                _det_group_by_cid[cid].append(pred)

        # image info
        img_info = {
            "file_name": batch["image_name"][i],
            "img_size": (
                to_numpy(batch["image_width"][i]),
                to_numpy(batch["image_height"][i]),
            ),
        ...
        if "Tr_vel2cam" in batch:
            img_info.update({"Tr_vel2cam": to_numpy(batch["Tr_vel2cam"][i])})
        else:
            img_info.update({"Tr_vel2cam": np.eye(4, dtype=np.float32)})
        if "Tr_vcs2cam" in batch:
            img_info.update({"Tr_vcs2cam": to_numpy(batch["Tr_vcs2cam"][i])})
        else:
            img_info.update({"Tr_vcs2cam": np.eye(4, dtype=np.float32)})
        if "index" in batch:
            img_info.update({"img_index": to_numpy(batch["index"][i])})
        else:
        ...
        for anno in annotations:
            ...
            if cid in _gt_group_by_cid:
                _gt_group_by_cid[cid].append(anno)
...
def match(
    ...
    for img_info in annotation["images"]:
        ...
        if len(all_gts[img_id]) == 0:
            for det in all_dets[img_id]:
                redundant_det.append(
                    [float(det["dep"]), float(det["rotation_y"])]
                )
            continue
        ...
        for gt in all_gts[img_id]:
            ...
            gt_yaw.append(yaw)
            hard_img_info.append([gt["category_id"], gt["depth"], 1, 360])
        ...
        if match_basis in ["det2d", "proj2d"]:
            gt_bboxes = np.clip(
                gt_bboxes,
                np.array([[0, 0, 0, 0]]),
                np.array(img_size * 2).reshape((1, 4)),
            )
            det_bboxes = np.clip(
                det_bboxes,
                np.array([[0, 0, 0, 0]]),
        ...
        for i in redundant:
            redundant_det.append(
                [
                    float(all_dets[img_id][i]["dep"]),
                    float(all_dets[img_id][i]["rotation_y"]),
                ]
            )
...
class Real3dEval(EvalMetric):
    ...
    Please see https://oshcjymfmk.feishu.cn/docs/doccntkQX0fu5a1YArq7AhqqRXc#w5AIuG    # noqa
    and http://wiki.hobot.cc/pages/viewpage.action?pageId=172397526 for more
    details.
    ...
    Args:
        ...
        metrics (tuple of str, default: None): Tuple of eval metrics, using
            ...
            error. For more metric details, refer to
            https://oshcjymfmk.feishu.cn/docs/doccntkQX0fu5a1YArq7AhqqRXc#e8nUDD    # noqa
    ...
    def __init__(
        ...
        *,
        ...
        depth_intervals: Optional[Sequence[int]] = (20, 50, 80, 120),
        ...
        save_name="real3d_metrics.xlsx",
        vis_eval_params: Optional[Dict] = None,
    ):
        ...
        self.vis_eval_params = vis_eval_params
        self.real3d_vis = None

        if (
            self.vis_eval_params is not None
            and self.vis_eval_params["vis_eval_results"]
        ):
            self.real3d_vis = Real3DVisualize(
                save_path=self.vis_eval_params["vis_path"],
                **self.vis_eval_params["vis_setting"],
            )
    ...
    def update(self, batch, outputs):
        ...
        if self.save_path:
            ...
            try:
                ...
                pass
        if self.real3d_vis is not None:
            self.real3d_vis.save_imgs(outputs, batch)
    ...
    def print_log(self, format_metrics, orientation):
        ...
        metric_single = OrderedDict()
        keys = []
        for line in log_line[1:-1]:
            cat_line = []
            for item in line.split(" "):
                if item == "":
                    continue
                cat_line.append(item)
            if "Camera+Category" in cat_line:
                for cat_metrics in cat_line:
                    if cat_metrics == "Camera+Category":
                        metric_single["Camera"] = []
                        metric_single["Category"] = []
                        keys.extend(["Camera", "Category"])
                        continue
                    metric_single[cat_metrics] = []
                    keys.append(cat_metrics)
            else:
                for cnt, item in enumerate(cat_line):
                    if "+" in item:
                        _item = item.split("+")
                        metric_single["Camera"].append(_item[0])
                        metric_single["Category"].append(_item[1])
                        continue
                    metric_single[keys[cnt + 1]].append(item)
        self.metric_single = pd.DataFrame(metric_single)
        try:
            current_time = time.strftime("%Y%m%d-%H%M", time.localtime())
            self.metric_single.to_excel(
                os.path.join(self.save_path, self.save_name),
    ...
    def convert_to_save_format(self, batch, output):
        ...
        for i in range(batch_size):
            ...
            meta = {
                ...
                "distCoeffs": to_numpy(batch["dist_coeffs"][i]),
                "img_size": to_numpy(
                    [batch["image_width"][i], batch["image_height"][i]]
                ),
            }
            if "image_transform" in batch:
                meta.update(
                    {
                        "c": np.array(
                            [
                                to_numpy(
                                    batch["image_transform"]["input_size"][0][
                                        i
                                    ]
                                ),
                                to_numpy(
                                    batch["image_transform"]["input_size"][1][
                                        i
                                    ]
                                ),
                            ]
                ...
                meta.update(
                    {
                        "s": np.array(
                            [
                                to_numpy(
                                    batch["image_transform"]["original_size"][
                                        0
                                    ][i]
                                ),
                                to_numpy(
            ...
            if "Tr_vcs2cam" in batch:
                meta.update({"Tr_vcs2cam": to_numpy(batch["Tr_vcs2cam"][i])})
