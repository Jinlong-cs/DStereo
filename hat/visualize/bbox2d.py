...
from hat.core.data_struct.base_struct import DetBoxes2D, MultipleBoxes2D
...
def draw_bbox(
    img: np.ndarray, bbox: List[float], color: Tuple[int], thickness: int
):
    """Draw rectangle box on img.

    Args:
        img: Image on which rectangle is to be drawn.
        bbox: Box, in [xmin, ymin, xmax, ymax] format.
...
def draw_text(
    ...
    Args:
        img: Image on which text is to be drawn.
        text: Text string to be drawn.
        coord: Coordinates of the bottom-left corner of the text
            string in the image, in (x_coordinate, y_coordinate) format.
        color: Color of text string to be drawn.
