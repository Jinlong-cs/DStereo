...
_det_task_name_list = {
    ...
    "tl_lens_detection," "traffic_cone_fast_detection",
}
...
_class_name_map = {
    ...
    "traffic_light_lens": "traffic_light_lens",
    "cyclist_2pe": "cyclist_2PE",
    "person_2pe": "person_2PE",
    "person_2PE": "person_2PE",
    "cyclist_2PE": "cyclist_2PE",
    "motocyclist_2PE": "motocyclist_2PE",
    "tricyclist_2PE": "tricyclist_2PE",
    "IR_Ramp": "IR_Ramp",
    "Other_GuideSign_WhiteBlack": "Other_GuideSign_WhiteBlack",
}
...
def get_class_names_used_in_desc(class_names):
    ...
    for name in _as_list(class_names):
        assert name in _class_name_map, (
            "can not fine %s in _class_name_map" % name
        )
...
def check_det_merge_fn_type_and_params(merge_fn_type, merge_fn_params):
    assert (
        merge_fn_type in _det_merge_fn_list
    ...
    for key in merge_fn_params:
        assert (
            key in _det_merge_fn_list[merge_fn_type]["key"]
...
def get_det_frcnn_merge_fn_type_and_params(
    crop_distance, w_or_h, obj_real_size
):  # noqa
...
def get_frcnn_det_desc(
    ...
    def _check():
        ...
        if conf_map_list is not None:
            assert isinstance(
                conf_map_list, (list, tuple)
            ), "expect list/tuple, but get %s" % type(conf_map_list)
            ...
            for conf_dict in conf_map_list:
                assert isinstance(
                    conf_dict, dict
                ), "expect dict, but get %s" % type(conf_map_list)
                assert (
                    "conf_value" in conf_dict
                    and "conf_percentage" in conf_dict
                )
                assert len(conf_dict["conf_value"]) == len(
                    conf_dict["conf_percentage"]
                ), "%d vs. %d" % (
    ...
    if roi_regions is not None:
        ...
        ) = get_det_frcnn_merge_fn_type_and_params(
            crop_distance, w_or_h, obj_real_size
        )
