...
from hat.core.box3d_utils import rotation_3d_in_axis
...
class BasePoints(object):
    ...
    Args:
        tensor: a N x points_dim matrix.
        points_dim: Number of the dimension of a point.
            Each row is (x, y, z). Defaults to 3.
        attribute_dims: Dictionary to indicate the
    ...
    def __init__(
        ...
        assert (
            tensor.dim() == 2 and tensor.size(-1) == points_dim
        ), tensor.size()
    ...
    @property
    def height(self):
        """torch.Tensor: A vector with height of each point in shape (N, 1), or None."""  # noqa: E501
        if (
            self.attribute_dims is not None
            and "height" in self.attribute_dims.keys()
        ):
    ...
    def height(self, tensor):
        ...
        if (
            ...
            if self.attribute_dims is None:
                self.attribute_dims = dict()  # noqa: C408
            attr_dim = self.shape[1]
            self.tensor = torch.cat([self.tensor, tensor.unsqueeze(1)], dim=1)
            self.attribute_dims.update(dict(height=attr_dim))  # noqa: C408
            self.points_dim += 1

    @property
    def color(self):
        """torch.Tensor: A vector with color of each point in shape (N, 3), or None."""  # noqa: E501
        if (
            self.attribute_dims is not None
            and "color" in self.attribute_dims.keys()
        ):
    ...
    def color(self, tensor):
        ...
        if (
            ...
            self.attribute_dims.update(
                dict(color=[attr_dim, attr_dim + 1, attr_dim + 2])  # noqa
            )
            self.points_dim += 3
    ...
    def rotate(self, rotation, axis=None):
        ...
        Args:
            rotation: Rotation matrix
                or angle.
            axis: Axis to rotate at. Defaults to None.
        """
    ...
    def in_range_3d(self, point_range: Tuple) -> torch.Tensor:
        """Check whether the points are in the given range.

        Args:
            point_range: The range of point
    ...
    def in_range_bev(self, point_range: Tuple) -> torch.Tensor:
        """Check whether the points are in the given range.
    ...
    def scale(self, scale_factor):
        ...
        Args:
            scale_factors: Scale factors to scale the points.
        """
    ...
    def new_point(
        self, data: Union[torch.Tensor, List, np.ndarray]
    ) -> "BasePoints":  # noqa
