...
try:
    from hatbc.message import Attribute, BBox2D, BBox3D, DenseMap, Instance
    from hatbc.message import Line2D as _Line2D
    from hatbc.message import Mask2D
    from hatbc.message import Point2D as _Point2D
    from hatbc.utils.compress import CompressMethod
except ImportError:
    Attribute, BBox2D, BBox3D, DenseMap, Instance = (
        None,
        None,
        None,
        None,
    ...
    _Line2D, Mask2D, _Point2D, CompressMethod = None, None, None, None
...
class ClsLabel(BaseData):
    ...
    def _check_values(self):
        ...
        if (
            not_none(self.score, self.cls_idx)
            and is_tensor(self.score)
    ...
    def to_aidi_eval(self):
        """Get results in AIDI Eval format."""
        return {
            "attrs": {
                "sub_type": self.cls_idx.item()
                if is_tensor(self.cls_idx)
                else self.cls_idx
            }
        }
...
class ClsLabels(BaseDataList):
    ...
    Args:
        cls_idxs: Shape(N), class category ids
    ...
    def inv_crop(self, crop_roi, image_hw):
        return self
    ...
    def with_cls_idxs_in(self, indices: Union[int, Sequence[int]]):
        ...
        assert (
            self.cls_idxs is not None
        ), "Boxes do not have cls_idxs attribute"
...
class DetBoxes2D(ClsLabels):
    ...
    def __post_init__(self, *args):
        ...
        if not_none(self.cls_idxs):
            assert (
                len(self.cls_idxs) == length and self.cls_idxs.device == device
            )
    ...
    def inv_crop(self, crop_roi, image_hw):
        self.boxes[:, 0::2] += crop_roi[0]
        self.boxes[:, 1::2] += crop_roi[1]
        return self
...
class MultipleBox2D(DetBoxes2D):
    def __post_init__(self, *args):
        super().__post_init__(*args)
        self.instance_attr = "instances"

    def to_aidi_eval(self):
        return [box.to_aidi_eval() for box in iter(self)]
...
class MultipleBoxes2D(BaseDataList):
    ...
    def inv_crop(self, crop_roi, image_hw):
        for boxes in iter(self):
            boxes.inv_crop(crop_roi, image_hw)
        return self
...
@dataclass
class DetBox2D3D(DetBox3D, DetBox2D):
    def to_aidi_eval(self):
        """Get results in AIDI Eval format."""
        return {
            "bbox_2d": self.box.numpy().tolist(),
            "depth": self.z.item(),
            "dimensions": self.dimension.numpy().tolist(),
            "rotation_y": self.yaw.item(),
            "location": self.location.numpy().tolist(),
...
@dataclass
class DetRotBoxes2D(ClsLabels):
    """Detection bbox3d data class for multiple samples."""

    # singleton_cls: ClassVar = DetBoxes2D
    h: torch.Tensor = None
    w: torch.Tensor = None
    x: torch.Tensor = None
    y: torch.Tensor = None
    yaw: torch.Tensor = None
    vcs_points: torch.Tensor = None
    ...
    @property
    def dimensions(self):
        return torch.stack([self.w, self.h], dim=-1)
...
@dataclass
class DetBoxes2D3D(DetBoxes3D, DetBoxes2D):
    singleton_cls: ClassVar = DetBox2D3D
...
class Mask(BaseData):
    ...
    def __post_init__(self):
        ...
        assert not torch.is_floating_point(self.mask)
    ...
    def inv_pad(self, *padding):
        h, w = self.mask.shape
        self.mask = self.mask[
            padding[2] : h - padding[3], padding[0] : w - padding[1]
        ]

    def inv_crop(self, crop_roi, image_hw):
        inv_mask = torch.full(image_hw, 255, dtype=self.mask.dtype).to(
            self.mask.device
        )
        inv_mask[
    ...
    @require_packages("hatbc")
    def to_hatbc_msg(self, topic: Optional[str] = None, instance: bool = True):
        data = self.mask.cpu()
        if instance:
            mask = Mask2D(
                topic=topic,
            ...
            mask.data.compress()
            return DenseMap(topic=topic, mask2ds=[mask])
...
class Masks(BaseDataList):
    ...
    def __post_init__(self):
        assert isinstance(self.masks, torch.Tensor)
        assert not torch.is_floating_point(self.masks)
    ...
    def inv_crop(self, crop_roi, image_hw):
        inv_mask = torch.zeros(
            (self.masks.shape[0], int(image_hw[0])),
            int(image_hw[1]),
...
def generate_nd_points(ndim: int):
    ...
    if ndim not in PTS_DICT:
        ...
        class PointsND(ClsLabels):
            ...
            def inv_crop(self, crop_roi, image_hw):
                if ndim == 2:
                    self.points[:, 0] += crop_roi[0]
                    self.points[:, 1] += crop_roi[1]
                else:
                    raise NotImplementedError
...
def generate_m_nd_points(m: int, ndim: int):
    ...
    if (ndim, m) not in PTS_DICT:
        ...
        class MPointsBase(BaseDataList):
            ...
            def inv_crop(self, crop_roi, image_hw):
                for i in range(self.num_points):
                    getattr(self, f"points{i}").inv_crop(crop_roi, image_hw)
                return self
        ...
        MPointsND = make_dataclass(
            ...
            bases=(MPointsBase,),
            namespace={
                "__post_init__": lambda x: check_lengths(
...
Point2D_2.to_aidi_eval = lambda self: {
    "attrs": {
        "p_WheelKeyPoints_2": [
...
class Line2D(Point2D_2):
    def __post_init__(self, *args):
        super().__post_init__(*args)
        self.instance_attr = "line2ds"

    def to_aidi_eval(self):
        res = super().to_aidi_eval()
    ...
    def to_hatbc_msg(self, topic: Optional[str] = None):
        return _Line2D(
            topic=topic,
            p0=_Point2D(
                data=self.point0.point.cpu(),
                score=self.point0.score.item(),
            ),
            p1=_Point2D(
                data=self.point1.point.cpu(),
                score=self.point1.score.item(),
            ),
        )
