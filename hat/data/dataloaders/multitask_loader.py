...
from hat.utils.apply_func import apply_to_collection
...
from hat.utils.logger import MSGColor, format_msg, rank_zero_info

__all__ = ["MultitaskLoader", "MultitaskInfLoader"]
...
class MultitaskLoader(object):  # noqa: D205,D400
    ...
    def __init__(
        ...
        Args:
            ...
            wrap_batch: Whether to wrap native batch to be {task_name: batch}
                so that MultitaskLoader can work with MultitaskGraphModel.
    ...
    def _wrap_sync_loaders_cycle(self):  # noqa: D400, D401
        ...
        _old_info = {
            k: get_dataloader_length(v) for k, v in self.loaders.items()
        }
        ...
        for k, v in self.loaders.items():
            logger.info(
                f"SyncLoaderLength {k}: {_old_info[k]} => {get_dataloader_length(v)}"  # noqa E501
            )
    ...
    def _wrap_loaders_max_size_cycle(self) -> Any:  # noqa: D205,D400
        ...
        for k, v in self.loaders.items():
            rank_zero_info(
                f"{k}: {_old_info[k]} => "
                f"{_old_info[k] if fill_none else get_dataloader_length(v) }"
            )  # noqa
...
class MultitaskInfLoader(object):
    def __init__(
        ...
        loaders: Dict,
        task_sampler: Optional[TaskSampler] = None,
        task_mapper: Optional[Dict] = None,
        return_task: bool = False,
        iter_once: bool = False,
        custom_length: Optional[int] = None,
    ):
        """Initialize method.
        ...
        Args:
            ...
            task_mapper:
                {
                    "vehicle_detection": "vehicle_full",
                    "vehicle_category_classification": "vehicle_full",
                }
                used in merged label training.
                mapping merged tasks into common task loader.
                when sampling a given task, if task in task_mapper,
                sample from common loader instead of task loader.
            return_task: return output as "batch, task" pairs, used by graph
                model input. Default return dict with task as key.
            iter_once: If set to True, dataloaders of all tasks will only be
                iterated once, and the length will be that of bigest subloader.
            custom_length: If the number of iteration exceeds this value, the
                loader will stop.
        """
        if task_sampler is not None:
            for task in task_sampler.tasks:
                assert (
                    task in loaders or task in task_mapper
                ), f"{task} not found in dataloader"

        self.loaders = {
            k: build_from_registry(v)
            for k, v in loaders.items()
            if k in task_sampler.tasks or task_mapper
        }
        ...
        self.loader_stopped = {k: False for k in loaders}
        if custom_length is not None:
            assert custom_length > 0
        self.custom_length = custom_length
        self.batch_i = 0
    ...
    def batch_size(self) -> int:
        if not self._batch_size:
            if isinstance(self.loaders, dict):
                if self.task_sampler and not self.task_mapper:
                    sampling_factors = {
                        k: self.task_sampler.config[k]["sampling_factor"]
                        for k in self.loaders
                    }
                else:
                    # If no task_sampler or merge label, use uniform sample.
                    sampling_factors = {k: 1 for k in self.loaders}
                self._batch_size = sum(
                    [
                        self.loaders[k].batch_size * sampling_factors[k]
    ...
    def __next__(self):
        if (
            ...
            and self.batch_i >= self.custom_length
        ):
            raise StopIteration
        if self.task_sampler is not None:
            tasks = self.task_sampler.sample_task()
            if isinstance(tasks, str):
                tasks = [tasks]
            if self.task_mapper:
                if self.iter_once:
                    raise NotImplementedError
                next_batch = self._sample_next(tasks)
            else:
                next_batch = OrderedDict(
                    (t, self._next_task(t)) for t in tasks
                )

        batch_task_pairs = []
        assert isinstance(next_batch, dict)
        for name, batch in next_batch.items():
            if batch is None:  # loader for this task is over
                if all(list(self.loader_stopped.values())):
                    raise StopIteration
                continue
            _name = name if self.return_task else None
            task_batch = {}
            task_batch.update(batch)
            if not self.task_mapper:
                task_batch[name] = batch
            elif name[0] not in self.task_mapper:
                task_batch[name[0]] = batch
                _name = name[0]
            else:
                for n in name:
                    if n not in task_batch.keys():
                        task_batch[n] = batch
    ...
    def _sample_next(self, tasks):
        """
        Sample next batch for given task.

        Map task to common loader if task in task mapper.

        Args:
            task: task name.
        """
        task_to_common = OrderedDict()
        for t in tasks:
            if t in self.task_mapper:
                common_t = self.task_mapper[t]
                if common_t in task_to_common:
                    task_to_common[common_t].append(t)
                else:
                    task_to_common[common_t] = [t]
            else:
                task_to_common[t] = [t]
        next_batch = OrderedDict()
        for common_t, t in task_to_common.items():
            next_batch[tuple(t)] = self._next_task(common_t)
        return next_batch
...
class MultitaskLoaderIterator(object):  # noqa: D205,D400
    ...
    def __next__(self) -> Any:
        ...
        if (
            ...
            raise StopIteration
        if self.task_sampler is not None:
            # TODO(min.du, 0.2): trainer has no `iter` attr anymore, get
        ...
        if self.return_task:
            ...
            for name, batch in next_batch.items():
                if self.wrap_batch:
                    # adopted from MultitaskInfLoader
                    task_batch = {}
                    if isinstance(batch, dict):
                        task_batch.update(batch)
                    task_batch[name] = batch
                    batch_task_pairs.append((task_batch, name))
                else:
                    batch_task_pairs.append((batch, name))
            next_batch = tuple(batch_task_pairs)
