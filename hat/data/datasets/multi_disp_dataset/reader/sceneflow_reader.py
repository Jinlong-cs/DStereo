...
class SceneFlowReader(BaseReader):
    def __init__(
        self, root, list_file, image_reader="PIL", disp_reader="PFM", right_disp=True
    ):
        super().__init__(
            root, list_file, image_reader, disp_reader, right_disp, occ_mask=False
        )
...
class FlyingThings3DSubsetReader(BaseReader):
    def __init__(
        ...
        super().__init__(
            root, list_file, image_reader, disp_reader, right_disp, occ_mask
        )
        assert (
            disp_reader == "PFM"
        ), "FlyingThings3DSubset Disp Reader only supports PFM format"
    ...
    def item_loader(self, item):
        ...
        if self.return_occ_mask:
            ...
            sample.update({"occ_mask": occ, "occ_mask_right": occ_right})
