...
class DenseboxDataset2MMDet(BASEDATE):
    """HAT DenseboxDataset interface for mmdetection.

    Args:
        dataset: cfg of HAT `DenseboxDataset`,
            Includes:
        ...
        classes: Specify classes to load.
        pipeline: Processing pipeline of mmdetection.
        test_mode: If set True, annotation will not be loaded.
        filter_empty_gt: If set true, images without bounding
            boxes of the dataset's classes will be filtered out. This option
            only works when `test_mode=False`. Defaults to True.
    """

    @require_packages("mmcv", "mmdet")
    def __init__(
        self,
        dataset: dict,
        ...
        datasets = []
        for hat_cfg in dataset_cfgs:
            assert isinstance(
                hat_cfg, dict
            ), f"element in `dataset` should be dict, but get {type(hat_cfg)}"  # noqa E501
            assert (
                hat_cfg.get("transform", None) is None
            ), f"`transform` should be None, but get {hat_cfg.get('transform')}"  # noqa E501
            task_type = hat_cfg.get("task_type", "detection")
            ...
            dataset = build_from_registry(hat_cfg)
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        Returns:
            Training/test data (with annotation if `test_mode` is set \
                True).
        """
        index = idx
    ...
    def _check_gt(self, ann):
        ...
        if self.task_type == "detection":
            ...
            # TODO(mengyang.duan): segmentation
            gt = None
    ...
    def _convert_format(self, data):
        """Convert data format from HAT to mmdetection."""
