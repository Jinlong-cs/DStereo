...
from ..avspeech.info import BucketAdaptor
...
def read_lists(list_file):
    ...
    Args:
        list_file: list file path.
    """
    lists = []
    with open(list_file, "r", encoding="utf8") as fin:
        lists = [BucketAdaptor.adapt(line.strip()) for line in fin]
    return lists


def read_non_lang_symbols(non_lang_sym_path):
    r"""Read non-linguistic symbol from file.
...
def read_symbol_table(symbol_table_file):
    ...
    Args:
        symbol_table_file: symbol table file path.
    """
    symbol_table = {}
