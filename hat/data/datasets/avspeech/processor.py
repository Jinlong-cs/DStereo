...
"""wenet processors and adapt functions.

adapt functions:
    collect_data_by_index : Collates the data according the index.
    adapt_data_dict       : Process data according the subsampling rate.
"""
...
def collect_data_by_index(
    ...
    """collect_data_by_index.

    Collates the data according the index.

    Args:
        data: data list.
        indexes: the order of the data to be retained in the input data.
    """
    ...
    for sample in data:
        ...
        assert len(sample) > max(
            indexes
        ), f"indexes: {indexes} out of range of sample: {len(sample)}"
        sample = [sample[idx] for idx in indexes]
        yield tuple(sample)


def adapt_data_dict(data, downsample_rate) -> Iterable:
    """adapt_data_dict.

    Process data according the subsampling rate,
    which is applicable to the single-mode data returned by the wenet dataset.
    ...
    for sample in data:
        ...
        for item in sample:
            item_length, _ = item["feat"].size()
            item_mask_length = item_length // downsample_rate
            item_length = item_mask_length * downsample_rate
            item["feat"] = item["feat"][:item_length, :]
            item["mask_length"] = item_mask_length
        ...
        mask_size = padded_feats.size(1) // downsample_rate
        mask_length = torch.tensor(
            [sample[i]["mask_length"] for i in order], dtype=torch.int32
        )
        tokens = [sample[i]["tokens"] for i in order]
        ret = {
            "keys": sorted_keys,
            "audio": padded_feats,
            "audio_lens": feats_length,
            "label": padding_labels,
            "label_length": label_lengths,
            "mask_size": mask_size,
            "mask_lens": mask_length,
            "tokens": tokens,
        }
        yield ret
...
@require_packages("torchaudio")
def compute_fbank(
    data, num_mel_bins=23, frame_length=25, frame_shift=10, dither=0.0
):
    """Extract fbank.
...
def spec_aug(
    ...
    for sample in data:
        ...
        if warp_for_time and max_frames > max_w * 2:
            ...
            y = torch.from_numpy(numpy.concatenate((left, right), 0)).to(
                x.device
            )
        # time mask
...
def static_batch(data, batch_size=16):
    """Make static batch the data by `batch_size`.
...
def padding(data):
    """Pad the data into training data.
