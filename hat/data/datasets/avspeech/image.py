...
"""AVSPEECH(多模语音)图像数据读取模块.

目前该模块下共有三个类:

BaseImageReader         : 图像数据读取基类.
TendisVideoImageReader  : 从tendis读取图像数据.
DirectVideoImageReader  : 从mp4文件读取图像数据.

其中, TendisVideoImageReader 和 DirectVideoImageReader
都继承了 BaseImageReader 类, 用于读取图像数据.
"""
...
class BaseImageReader(object):
    ...
    图片读取的基类, 实现补充缺失图片的基本方法. 子类需要实现 read_seg_images 接口.
...
class TendisVideoImageReader(BaseImageReader):
    """TendisVideoImageReader.

    该类实现了从tendis数据库读取图像数据的接口.

    Args:
        fps: 目标图像帧率, 默认为30.
        origin_fps: 原始图像帧率.
        zfill_num: 整数规范化的位数, 默认为6.
        missing_image_complete_mode: 补全丢失图片的模式, 默认为"zero.
        filter_rules: 数据过滤策略，当满足数据过滤的策略时需要删除数据.
    ...
    def __init__(
        ...
        super(TendisVideoImageReader, self).__init__(
...
@OBJECT_REGISTRY.register
class DirectVideoImageReader(BaseImageReader):
    """DirectVideoImageReader.

    该类实现了直接从mp4文件读取图像数据的接口.
    ...
    def __init__(
        ...
        super(DirectVideoImageReader, self).__init__(
            missing_image_complete_mode=missing_image_complete_mode,
            filter_rules=filter_rules,
        )
        self._fps = fps
    ...
    def read_seg_images(
        self, mp4_path: str, logger: logging.Logger = logger
    ) -> Optional[List[np.ndarray]]:
        """read_seg_audio.
        ...
        for _ in range(3):
            img_lst = []
            try:
                vcap = cv2.VideoCapture(mp4_path)
                ori_fps = vcap.get(cv2.CAP_PROP_FPS)
                count = int(vcap.get(cv2.CAP_PROP_FRAME_COUNT))
                logger.debug(f"{mp4_path} fps {ori_fps} count {count}")
                while vcap.isOpened():
        ...
        if len(img_lst) == 0:
            return None
        # 将图片变成目标 fps
        if abs(ori_fps - self._fps) > 1:
            target_count = math.ceil(count * self._fps / ori_fps)
            img_lst = [
                img_lst[int(idx)]
                for idx in np.linspace(
                    0, len(img_lst), target_count, endpoint=False
                )
            ]
        return img_lst
