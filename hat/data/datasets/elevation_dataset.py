...
from hat.utils.pack_type.mxrecord import MXRecord, unpack, unpack_img
...
def get_gt(
    ...
    depth_gt = depth_gt.astype(np.float64) / DEPTH_COVER_SCALE - 1.0
    height_gt = height_gt.astype(np.float64) / HEIGHT_COVER_SCALE - 1.0
    gamma_gt = gamma_gt.astype(np.float64) / GAMMA_COVER_SCALE - 1.0
...
# TODO: (teng.chen) AutoElevation will be deprecated when rec data enough.
...
class ReaderElevation(object):
    ...
    def _load_rec(self, rec_path: str):
        if rec_path:
            ...
            image_rec = MXRecord(rec_path, idx_path=idx_path, writable=False)
        else:
            image_lst, image_rec = None, None
        return image_lst, image_rec

    def _load_lmdb(self, lmdb_path: str):
        lmdb = Lmdb(lmdb_path, False, readonly=True)
        return lmdb
    ...
    def __setstate__(self, state):
        ...
        if self.ele_lmdb_path is not None:
            self.ele_lmdb = Lmdb(self.ele_lmdb_path, False, readonly=True)
        if self.intrinsics_lmdb_path:
            self.intrinsics_lmdb = Lmdb(
                self.intrinsics_lmdb_path, False, readonly=True
            )
    ...
    def get_rec_img(self, rec: str, idx: str):
        _, img = unpack_img(rec.read(idx), cv2.IMREAD_COLOR)
        return img

    def get_rec_gt(self, rec: str, idx: str):
        header, img = unpack(rec.read(idx))
