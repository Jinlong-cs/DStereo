...
class VehicleFlankRoiTransform:
    ...
    def __call__(self, data):
        ...
        bbox_clses = np.array(bbox_clses, np.int64).flatten()  # [n]
        flanks, flank_clses = [], []
        ...
        flank_clses = np.array(flank_clses, np.int64).flatten()  # [n]
...
@OBJECT_REGISTRY.register
class VehicleFlankAffineAugTransform(object):
    def __init__(
        self,
        target_wh,
        scale_type="W",
...
class PadFlankData(object):
    ...
    def __call__(self, data):
        ...
        pad_shape[0] = self.max_gt_boxes_num
        data["gt_flanks_num"] = (
            np.array(data["gt_flanks"].shape[0])
            .reshape((1,))
            .astype(np.float32)
        )
