...
# Source code reference to NVIDIA/DeepLearningExamples.
# https://github.com/NVIDIA/DeepLearningExamples/blob/master/PyTorch/Classification/ConvNets/image_classification/gpu_affinity.py  # noqa
...
def set_socket_single_affinity(gpu_id, nproc_per_node, cores):
    ...
    list of all CPU physical cores from the CPU socket connected to the GPU
    with a given id.
...
def set_socket_single_unique_affinity(gpu_id, nproc_per_node, cores):
    ...
    from the list of all CPU cores from the CPU socket connected to the GPU
    with a given id.
...
def set_affinity(
    ...
    * 'all_logical' - assigns the process with all logical cores associated
    with a given corresponding physical core (i.e., automatically includes all
    ...
    * 'single_logical' - assigns the process with only one logical core
    ...
    to GPUs, so on DGX A100, it will limit the code to half of the CPU cores
    and half of CPU memory bandwidth (which may be fine for many DL models).
